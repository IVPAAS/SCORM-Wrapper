...
playbar.PlayBarSlider.prototype = 
{
	...
	onMouseDown : function(event)
	{
		...
		var frame = Math.round((pos*playbar.movieProperties.project_main.to)/progDiv.width);
		if(frame < 1)
			frame = 1;
	...
	moveSlider : function(event)
	{
		...
		var frame = Math.round((posX*playbar.movieProperties.project_main.to)/this.width);
		if(frame < 1)
			frame = 1;
		else if(frame>playbar.movieProperties.project_main.to)
			frame = playbar.movieProperties.project_main.to;
	...
	updateSlider : function(frame)
	{
		...
		progDiv.thumbDiv.title = (cpInfoCurrentSlideIndex + 1) + "/" + cp.movie.stage.slides.length;
	}
};
...
playbar.playBarConstruct.prototype = 
{
	...
	applyColor : function(color,imgElem)
	{
		...
		setFillStrokeStyle( color,colorCanvas,undefined,true);
		...
		destcontext.globalAlpha = color.alpha/100;

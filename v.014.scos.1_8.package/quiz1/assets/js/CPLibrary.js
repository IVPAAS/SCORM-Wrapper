...
(function () {
	...
	{   
		...
		return (version >= 534) && (cp.device == cp.IDEVICE);   
	}
	...
	cp.animateItem = function(item,effectId,continueProj)
	{
		...
		if(itemData)
		{
			...
			var lEffectAlreadyAdded = false;
			for(var l = 0; l < itemData.runningEffects.length; ++l)
			{
				if(itemData.runningEffects[l] == effectId)
				{
					lEffectAlreadyAdded = true;
					break;
				}				
			}
			if(!lEffectAlreadyAdded)
	...
	cp.redrawItem = function(itemName)
	{
	...
	cp.HandleInteractiveWidget = function(aEvent,aCanvasName)
	{
		if(undefined == aCanvasName)
			return;
		var displayObj = displayObjectMap[aCanvasName];
		if(displayObj && displayObj.HandleClickEvent)
			displayObj.HandleClickEvent(aEvent);			
	}
	...
	cp.cbKH = function(obj, objc, isCorrectKey)
	{
		...
		if ( isCorrectKey ) {
			if(lObjId)
				SubmitInteractions(lObjId, cp.QuestionStatusEnum.CORRECT, lCurrentAttempt);
		...
		if(lObjId)
			SubmitInteractions(lObjId, cp.QuestionStatusEnum.INCORRECT, lCurrentAttempt-1);	
	...
	cp.handleDblClick = function( event )
	{
		...
		var clickData = null;
		var lScaledPosition = getScaledPosition(getPageX(event), getPageY(event));
		...
		if ( clickData && clickData.m_obj && ! clickData.m_obj.actionInProgress ) 
		{
			...
			if(lObjC!=undefined)
				lObjId = lObjC.dn;
			if(lObjId)
				SubmitInteractions(lObjId, cp.QuestionStatusEnum.INCORRECT, currentAttempt-1);
    ...
    cp.QuestionStatusEnum = new Object();
	...
	cp.Question = function(questionObjName,associatedObjName)
	{
		...
		if (!this['QuestionStatusEnum'])
		{
			...
			this.QuestionStatusEnum.INCOMPLETE = cp.QuestionStatusEnum.INCOMPLETE;
			this.QuestionStatusEnum.INCORRECT = cp.QuestionStatusEnum.INCORRECT;
			this.QuestionStatusEnum.CORRECT = cp.QuestionStatusEnum.CORRECT;
			this.QuestionStatusEnum.PARTIAL_CORRECT = cp.QuestionStatusEnum.PARTIAL_CORRECT;
		}
	...
	cp.Question.prototype = 
	{
		...
		{
			//DO NOTHING. SPECIAL CASES TO BE HANDLED IN RESPECTIVE QUESTIONS.
			...
			else
			{
				...
				else 
				{
					...
					else
					{
						...
						{
                            var lIdx = 0;
                            if(this.incorrectFeedbackCaptions)
                                lIdx = this.incorrectFeedbackCaptions.length - 1;

							lReturnFeedbackObject = this.getFailureFeedback(lIdx);
						}
	...
	cp.HotspotQuestion.prototype.registerHotspotInput = function(iHotspotInput)
	{
		...
		for(var i = 0; i < this.answerOptions.length; ++i)
		{
			var lCanvasName = this.answerOptions[i];
			if(lCanvasName == iHotspotInput.parentDivName)
			{				
				this.hotspotInputs[lCanvasName] = iHotspotInput;
			}			
		}		
		for(var i = 0; i < this.answerOptions.length; ++i)
		{
			var lCanvasName = this.answerOptions[i];
			if(this.hotspotInputs[lCanvasName] == undefined)
				return;			
		}
		
		this.drawHotspotsAgain();
	}
	...
	{
		this.resetHotspotPositionList(true);
		this.drawHotspots(true);
	}
	
	cp.HotspotQuestion.prototype.drawHotspots = function(iDontModifyHotspotPositionList)
	{
		...
		for(var i = 0; i < totalPositions; ++i)
		{
			...
			this.addHotspotDiv(currSlide,currHotspotPositionArr[0],currHotspotPositionArr[1],currHotspotPositionArr[2],iDontModifyHotspotPositionList);
		}			
	}
	...
	cp.HotspotQuestion.prototype.resetHotspotPositionList = function(iJustRemoveChild)
	{	
		if(!iJustRemoveChild)
			this.hotspotPositionList = [];
	...
	cp.HotspotQuestion.prototype.addHotspotDiv = function(currDiv,x,y,isCorrect,iDontModifyHotspotPositionList)
	{
		var newImg = document.createElement('img'); 
		...
		newImg.id = "hs_" + x + y;
		cp.redrawItem(newImg.id);
		if(!iDontModifyHotspotPositionList)
	...
	cp.MatchingQuestion = function(questionObjName,associatedObjName)
	{
		cp.MatchingQuestion.baseConstructor.call(this, questionObjName, associatedObjName);
		...
		this.column1OptionsList = new Array();
	}
	...
	{
		var lCol1Options = [];
		var lOptions = this.questionData['aio'];
		if(!lOptions || (lOptions.length <=0))
			return "";
		for(var i =0; i< lOptions.length; ++i)
		{
			var lOpt = lOptions[i].split(":"); // Syntax option = "itemCanvasName:AnswerIndex"
			lCol1Options.push(lOpt[0]);
		}
		return lCol1Options;
	}
	...
	{
		var lCol2Options = [];
		var lOptions = this.questionData['aco'];
		if(!lOptions || (lOptions.length <=0))
			return "";
		for(var i =0; i< lOptions.length; ++i)
		{
			var lOpt = lOptions[i].split(":"); // Syntax option = "itemCanvasName:AnswerIndex"
			lCol2Options.push(lOpt[0]);
		}
		return lCol2Options;
	}
	
	cp.MatchingQuestion.prototype.getXYForColumn2Option = function(iAnswerID, lFromElem)
	{		
		for(var i = 0; i < this.Column2Options.length; ++i)
		{
			var lData = cp.model.data[this.Column2Options[i]];
			if((lData.aid == iAnswerID) || (lData.aid + ")" == iAnswerID))
			{
				var retXYObj = new Object();
				var column2OptionElem = document.getElementById(this.Column2Options[i]);
				if(column2OptionElem)
				{					
					retXYObj.x = (parseFloat(column2OptionElem.parentElement.style.left));// + "px";
					retXYObj.y = parseFloat(column2OptionElem.parentElement.style.top) + parseFloat(column2OptionElem.parentElement.style.height)/2;// + "px";	
					return retXYObj;
				}				
				else
				{
					if(cp.verbose)
					{
						cp.log("Elem not found. Adding it to drawLaterMap : " + this.Column2Options[i]);
						cp.log("Elem not found. Adding it for : " + lFromElem.id);
					}
					if(this.drawLaterMap[this.Column2Options[i]] == undefined)
					{
						this.drawLaterMap[this.Column2Options[i]] = {};						
					}
					this.drawLaterMap[this.Column2Options[i]][lFromElem.id] = lFromElem.id;
	...
	cp.MatchingQuestion.prototype.startQuestion = function()
	{
		...
		if(lIsInReviewMode)
			this.addCorrectAnswerDivs();
			
		cp.MatchingQuestion.superClass.startQuestion.call(this);
	}
	
	cp.MatchingQuestion.prototype.addCorrectAnswerDivs = function()
	{
		if(!this.getWasJudged())
			return;
		...
		for(var key in this.Column1OptionsMap)
		{
			var lOptionElem = this.Column1OptionsMap[key];
			lOptionElem.addReviewButton();			
		}
	}
	
	cp.MatchingQuestion.prototype.showReviewArea = function(divElement)
	{		
		for(var key in this.Column1OptionsMap)
		{
			var lOptionElem = this.Column1OptionsMap[key];
			lOptionElem.removeOpenReviewAreas();
		}
		...
		var seqReviewCloseButtonElem = document.getElementById('matchingReviewCloseButton');
		seqReviewCloseButtonElem.onclick = function(e) 
										{
											currSlide.parentNode.removeChild(divElement);
											parentSlideDivElement.style.cursor = 'default';
	...
	cp.MatchingQuestion.prototype.registerColumn1Option = function(iElement)
	{		
		for(var i = 0; i < this.Column1Options.length; ++i)
		{
			var lCanvasName = this.Column1Options[i];
			var lCanvasData = cp.model.data[lCanvasName];
			var lDivName = lCanvasData.dn;			
			
			var lElemData = cp.model.data[iElement.id];
			if(lElemData.answerTextDivName == lCanvasName)
			{
				this.Column1OptionsMap[lCanvasName] = iElement;
				return;
			}			
		}
	}
	
	cp.MatchingQuestion.prototype.registerColumn2Option = function(iElement)
	{
		for(var i = 0; i < this.Column2Options.length; ++i)
		{
			var lColumn2OptionName = this.Column2Options[i];
			var lCanvasData = cp.model.data[lColumn2OptionName];
			var lDivName = lCanvasData.dn;
			if(iElement.id.indexOf(lDivName) != -1)
			{				
				for(var key in this.drawLaterMap)
				{					
					if(lColumn2OptionName == key)
					{
						var lColumn1OptionNames = this.drawLaterMap[key];							
						for(var columnName in lColumn1OptionNames)
						{
							var lFromElem = document.getElementById(columnName);
							if(lFromElem)
							{
								var lPosObj = new Object();
								
								lPosObj.x = parseFloat(iElement.parentElement.style.left) + parseFloat(iElement.style.left);// + "px";
								lPosObj.y = parseFloat(iElement.parentElement.style.top) + parseFloat(iElement.parentElement.style.height)/2;// + "px";	

								lFromElem.drawLine(lPosObj);
							}
						}
					}					
				}
				this.Column2OptionsMap[lColumn2OptionName] = iElement;
				return;
			}
		}
	}
	...
	cp.MatchingQuestion.prototype.clearAnswers = function()
	{
		...
		//Clear all answer related arrays		
		
		for(var i = 0; i < this.Column1Options.length; ++i)
		{
			var lElem = this.Column1OptionsMap[this.Column1Options[i]];
			if(lElem)
				lElem.resetMatch();
		}		
	}	
	
	cp.MatchingQuestion.prototype.addDragEvents = function(iDivName)
	{
		if(cp.verbose) 
			cp.log("Adding drag events : " + iDivName);
		
		var elem = document.getElementById(iDivName);
		var currSlide = document.getElementById(this.getAssociatedObjName());
		this.lDraggableItem;
		...
		if(cp.DESKTOP != cp.device)
		{
			...
			{					
				...
				var lTouch = touches[0];
				self.lDraggableItem = lTouch.target;
				self.lIsColumn1ItemDragged = false;
				for(var i = 0; i < self.Column1Options.length; ++i)
				{
					var lCanvasData = cp.model.data[self.Column1Options[i]];
					var lDivName = lCanvasData.dn;
					if(self.lDraggableItem.id.indexOf(lDivName) != -1)
					{					
						self.lDroppableItemsMap = self.Column2OptionsMap;
						self.lIsColumn1ItemDragged = true;
					}
				}
				
				if(!self.lIsColumn1ItemDragged)
				{
					for(var j = 0; j < self.Column2Options.length; ++j)
					{
						var lCanvasData = cp.model.data[self.Column2Options[j]];
						var lDivName = lCanvasData.dn;
						if(self.lDraggableItem.id.indexOf(lDivName) != -1)
						{						
							self.lDroppableItemsMap = self.Column1OptionsMap;
						}
					}
				}
				
				var divData = cp.model.data[self.lDraggableItem.id];
				var canvasName = divData.answerTextDivName;
				
				var scaledTouchPos = getScaledPosition(lTouch.pageX, lTouch.pageY);
				...
				self.draggedImageCopy.style.left = (scaledTouchPos.X + 10) + "px";
				self.draggedImageCopy.style.top = scaledTouchPos.Y + "px";
				self.draggedImageCopy.style.width = (lCanvasData.b[2] - lCanvasData.b[0]) + "px";
				self.draggedImageCopy.style.height = (lCanvasData.b[3] - lCanvasData.b[1]) + "px";
				self.draggedImageCopy.style.display = "block";
				self.draggedImageCopy.style.position = "absolute";	
			...
			elem.ontouchend = function(e)
			{
				...
				var draggedObj = new Object();
				draggedObj.minX = parseFloat(self.draggedImageCopy.style.left);
				draggedObj.minY = parseFloat(self.draggedImageCopy.style.top);
				draggedObj.maxX = parseFloat(self.draggedImageCopy.style.left) + parseFloat(self.draggedImageCopy.style.width);
				draggedObj.maxY = parseFloat(self.draggedImageCopy.style.top) + parseFloat(self.draggedImageCopy.style.height);
				
				self.draggedImageCopy = undefined;
				
				var isDroppedOver = function(iDraggedObj,iDroppedOverObj)
				{
					if((iDraggedObj.minX > iDroppedOverObj.maxX) || (iDraggedObj.minY > iDroppedOverObj.maxY)
						|| (iDraggedObj.maxX < iDroppedOverObj.minX) || (iDraggedObj.maxY < iDroppedOverObj.minY))
						return false;

					return true;
				}

				for(var key in self.lDroppableItemsMap)
				{
					var lDroppableItem = self.lDroppableItemsMap[key];
					var droppedOverObj = new Object();
					droppedOverObj.minX = parseFloat(lDroppableItem.style.left) + parseFloat(lDroppableItem.parentElement.style.left);
					droppedOverObj.minY = parseFloat(lDroppableItem.style.top) + parseFloat(lDroppableItem.parentElement.style.top);
					droppedOverObj.maxX = parseFloat(lDroppableItem.parentElement.style.width) + droppedOverObj.minX;
					droppedOverObj.maxY = parseFloat(lDroppableItem.parentElement.style.height) + droppedOverObj.minY;
					
					if(isDroppedOver(draggedObj,droppedOverObj))
					{
						if(cp.verbose)
							cp.log("Dropped on : " + lDroppableItem.id);
						
						var lCol1ItemToBeSet;
						var lCol2ItemName;
						if(self.lDraggableItem.drawLine)
						{
							var lDroppedImageData = cp.model.data[key];
							self.lDraggableItem.drawLine(self.getXYForColumn2Option(lDroppedImageData.aid,self.lDraggableItem));
							lCol1ItemToBeSet = self.lDraggableItem.oHandler;
							lCol2ItemName = key;
						}
						else
						{
							var lDraggedImageData = cp.model.data[self.lDraggableItem.id];
							lDroppableItem.drawLine(self.getXYForColumn2Option(lDraggedImageData.aid,lDroppableItem));
							lCol1ItemToBeSet = lDroppableItem.oHandler;							
							var lCol2ItemDivData = cp.model.data[lDraggedImageData.actid];
							lCol2ItemName = lCol2ItemDivData.mdi;
						}
						
						for(var i = 0; i < self.Column2Options.length; ++i)
						{
							if(lCol2ItemName == self.Column2Options[i])
							{
								lIndex = i;
								break;
							}
						...
						lCol1ItemToBeSet.setIndex(lIndex);
					}
				}
			};
		}
		else
		{
			...
			elem.onmousedown = function(e)
			{
				...
				var scaledTouchPos = getScaledPosition(e.clientX, e.clientY);
	...
	cp.MatchingQuestion.prototype.removeDragEvents = function(iDivName)
	{
		if(cp.verbose)
			cp.log("Removing drag events for : " + iDivName);
		
		var divElem = document.getElementById(iDivName);
		if(!divElem)
			return;
	...
	cp.MatchingQuestion.prototype.shuffleAnswers = function()
	{
		...
		if((!lCanShuffle || this.m_isShuffled) && (this.Column1Options.length > 0))
			return;
				
		var i;
		var len = this.Column1Options.length;
		var n;
		var temp;
		var RandomAnswers_array = new Array();
		...
		for(i = 0; i < len ; i++)
		{
			RandomAnswers_array[i] = this.Column1Options[i];
		}
		...
		this.shuffledColumn1Options = RandomAnswers_array;
		this.m_isShuffled	= true;
	}
	...
	cp.MatchingQuestion.prototype.getSelectedIndex = function(iAnswerLabelId)
	{
		var retVal = -1;
		...
		if(this.answerLabels)
		{
			...
			{
				var lAnswerLabel = this.answerLabels[i];
				var lAnswerLabelData = cp.model.data[lAnswerLabel];
				if(iAnswerLabelId == lAnswerLabelData.aid)
					return i;
			}
		}
	...
	cp.MatchingQuestion.prototype.getAnswerOption = function(column1OptionName)
	{
		var answerOptionData;
		
		if(!this.column1OptionsList || (this.column1OptionsList.length <= 0) || (this.column1OptionsList.length >= this.shuffledColumn1Options.length))
			this.column1OptionsList = new Array();
			
		this.column1OptionsList.push(column1OptionName);
		if(this.shuffleCounter >= this.shuffledColumn1Options.length)
			this.shuffleCounter = -1;
			
		var lCurrentOption = new Object();
		this.shuffleCounter++;
		lCurrentOption.name = this.shuffledColumn1Options[this.shuffleCounter];
		answerOptionData = cp.model.data[lCurrentOption.name];
		var column1OptionDivData = cp.model.data[column1OptionName];
		if(answerOptionData != undefined)
		{	
			//answerOptionData['aid'] = column1OptionDivData['aid'];	
		}	
		
		if(this.m_selectedAnswersArr && this.m_selectedAnswersArr.length > 0)
		{
			var lIndex = -1;
			for(var key = 0; key < this.Column1Options.length; ++key)
			{
				if(this.Column1Options[key] == lCurrentOption.name)
					lIndex = key;
			}
			lCurrentOption.selectedAnswerLabel = this.m_selectedAnswersArr[lIndex];
		}
		
		if(this.shuffleCounter == this.shuffledColumn1Options.length - 1)
			this.answerOptionsDrawn = true;
			
		return lCurrentOption;	
	}
	
	cp.MatchingQuestion.prototype.getIsOptionCorrect = function(iElem,iSelectedIndex)
	{		
		if(iSelectedIndex == -1)
			return false;
			
		var lSelectedOptionLabelCanvasName = this.Column2Options[iSelectedIndex];
		var lSelectedOptionLabelCanvasData = cp.model.data[lSelectedOptionLabelCanvasName];
			
		var elemData = cp.model.data[iElem.id];
		var answerTextElemData = cp.model.data[elemData.answerTextDivName];
			
		if(lSelectedOptionLabelCanvasData.aid == answerTextElemData.aid)
			return true;
	...
	{		
		...
		for(var i = 0; i < this.Column1Options.length; ++i)
		{			
			var lElemName = this.Column1Options[i];
			var lElem = this.Column1OptionsMap[lElemName];
			if(lElem)
			{
				this.removeDragEvents(lElem.id);
				lElem.disableOption();
				if(this.answerOptionsDrawn == true)
					lElem.oHandler.disable();
			}	
		}

		for(var i = 0; i < this.Column2Options.length; ++i)
		{			
			var lElemName = this.Column2Options[i];
			var lElem = this.Column2OptionsMap[lElemName];
			if(lElem)
			{
				this.removeDragEvents(lElem.id);
				lElem.disableOption();
	...
	cp.MatchingQuestion.prototype.checkIfAttempted = function()
	{
		...
		{
			var lOptionElem = this.Column1OptionsMap[key];
			if(lOptionElem.oHandler.getIndex() != -1)
				return true;
		}
	...
	cp.MatchingQuestion.prototype.checkIfCorrect = function()
	{
		...
		{
			var lOptionElem = this.Column1OptionsMap[key];
			var lSelectedIndex = lOptionElem.oHandler.getIndex();
			if(lSelectedIndex == -1)
				return false;
			
			var lSelectedOptionLabelCanvasName = this.Column2Options[lSelectedIndex];
			var lSelectedOptionLabelCanvasData = cp.model.data[lSelectedOptionLabelCanvasName];
			
			var elemData = cp.model.data[lOptionElem.id];
			var answerTextElemData = cp.model.data[elemData.answerTextDivName];
	...
	cp.MatchingQuestion.prototype.getAnswerScores = function()
	{
		...
		{			
			var lElemName = this.Column1Options[i];
			var lOptionElem = this.Column1OptionsMap[lElemName];
			
			if(lOptionElem == undefined)				//should never be true
				continue;
			...
			var lAnsScore = new cp.AnswerScore();
			lAnsScore.m_answerID = ctr.toString();
			lAnsScore.m_correctAnswer = answerTextElemData.aid;	
			...
			else
			{
				var lSelectedOptionLabelCanvasName = this.Column2Options[lSelectedIndex];
				var lSelectedOptionLabelCanvasData = cp.model.data[lSelectedOptionLabelCanvasName];				
				lAnsScore.m_chosenAnswer = lSelectedOptionLabelCanvasData.aid;
			}			
	...
	cp.MatchingQuestion.prototype.setQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
			
		aQuestionSpecificScore.m_answerOrderArrayAsString = this.m_answerOrderArray;//this.m_answerOrderArray is actually a string in this case. Not modifying the variable name as of now.
	},

	cp.MatchingQuestion.prototype.restoreFromQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
	...
	cp.MatchingQuestion.prototype.setAnswerOrder = function(iOrder)
	{
		if(!iOrder || iOrder.length <= 0)
			return;
		...
		var lAnySelectedAnswer = true;
		var iOrderArr = iOrder.split(";");
		var lItemLabels = this.questionData.itemlb;
		this.shuffledColumn1Options = new Array(this.Column1Options.length);
		for(var i = iOrderArr.length - 1; i >= 0 ; --i)
		{
			var lXYcoord = iOrderArr[i].split(":");
			var lXcoord = lXYcoord[0];
			var lYcoord = lXYcoord[1];
			for(var j = 0; j < lItemLabels.length; ++j)
			{
				var lColumn1OptionLabelData = cp.model.data[lItemLabels[j]];
				var lBounds = lColumn1OptionLabelData.b;
				if(lBounds[0] == lXcoord && lBounds[1] == lYcoord)
				{
					this.shuffledColumn1Options[j] = this.Column1Options[i];
				}
			}			
		}
		
		this.answerOptions = iOrderArr.slice(0);				
		this.m_answerOrderArray = iOrder;//Arr.slice(0);
	}
	
	cp.MatchingQuestion.prototype.resumeSelectedAnswers = function(iAnswerScores)
	{				
		if(iAnswerScores.length <= 0)
			return;		
		...
		for (var whichAns = 0; whichAns <iAnswerScores.length; whichAns++)
		{
			var lAnsID = unescape(iAnswerScores[whichAns].m_chosenAnswer);
			if(lAnsID == "0")
			{
				this.m_selectedAnswersArr.push(undefined);
				continue;
			}
			
			this.m_selectedAnswersArr.push(lAnsID);
		}		
	}	
	...
	cp.MatchingQuestion.prototype.setSelectedAnswers = function()
	{
		...
		{			
			...
			this.m_selectedAnswersArr.push(lSelectedOptionLabelCanvasData.aid);
		}		
	}
	...
	cp.MatchingQuestion.prototype.getChosenAnswerAsString = function()
	{
		...
		{			
			...
			var lSelectedIndex = lOptionElem.oHandler.getIndex();
			//if(lSelectedIndex == -1)
			//	return "";//in case if any of the answer is incomplete, return empty string - BUG IN SWF
			...
			if(ctr == 0)
			{
				ret_Str += ctr + "." + (lSelectedOptionLabelCanvasData ? lSelectedOptionLabelCanvasData.aid : " ");
			}
			else
			{
				ret_Str += "," + ctr + "." + (lSelectedOptionLabelCanvasData ? lSelectedOptionLabelCanvasData.aid : " ");	
	...
	cp.MatchingQuestion.prototype.getCorrectAnswerAsString = function()
	{
		...
		{			
			...
			if(ctr == 0)
			{
				ret_Str += ctr + "." + answerTextElemData.aid;
			}
			else
			{
				ret_Str += "," + ctr + "." + answerTextElemData.aid;	
	...
	{
		var lLeftColumnIDToTextMap = {};
		var lRightColumnIDToTextMap = {};
		var lAllDesc = new Object();
		...
		{			
			...
			lLeftColumnIDToTextMap[ctr++] = escape(answerTextElemData.atxtlms);			
		}
		for(var j = 0; j < this.Column2Options.length; ++j)
		{			
			var lElemName = this.Column2Options[j];
			var lAnswerElem = this.Column2OptionsMap[lElemName];
			
			if(lAnswerElem == undefined)				//should never be true
				continue;
			
			var elemData = cp.model.data[lAnswerElem.id];
			var answerTextElemData = cp.model.data[elemData.answerTextDivName];
			
			lRightColumnIDToTextMap[answerTextElemData.aid] = escape(answerTextElemData.atxtlms);			
		}
		
		lAllDesc.left = lLeftColumnIDToTextMap;
		lAllDesc.right = lRightColumnIDToTextMap;		
	...
	{
		var lAnswers = new Array();
		var lAnswerOrder = "";
		var i = 0;
		for(var key = 0; key < this.Column1Options.length; ++key)
		{
			var lOptionElem = document.getElementById(this.Column1Options[key]);
			if(i == 0)
			{
				lAnswerOrder += parseFloat(lOptionElem.parentElement.style.left);
				lAnswerOrder += ":";
				lAnswerOrder += parseFloat(lOptionElem.parentElement.style.top);
			}
			else
			{
				lAnswerOrder += ";";
		...
		this.m_answerOrderArray = lAnswerOrder;
		
		this.setSelectedAnswers();
	}	

	cp.FIBQuestion = function(questionObjName,associatedObjName)
	{
		cp.FIBQuestion.baseConstructor.call(this, questionObjName, associatedObjName);
		
		this.answerOptions = this.getAnswerOptions();
		this.answerOptionToAnswersMap = this.createAnswerOptionToAnswersMap();//this should be updated on resume from LMS
		this.defaultTitleImage = this.questionData.defaultTitleImage;
	}
	...
	cp.FIBQuestion.prototype.resetQuestionData = function()
	{
		...
		{
			this.m_selectedAnswersArr = [];
			for( var key in this.answerOptionToAnswersMap )
			{
				var lAnsObj = this.answerOptionToAnswersMap[key];
				if(lAnsObj.selectedAns)
					lAnsObj.selectedAns = "";
			}	
		}		
	}
	...
	{
		var lAnswerOptionsMap = {};
		for(var i = 0; i < this.answerOptions.length; ++i)
		{
			var lAnswerOptionName = this.answerOptions[i];			
			var lAnswerOptionData = cp.model.data[lAnswerOptionName];
			var lCorrectAnswers = lAnswerOptionData.correctAnswers;
			
			var lAnsObj = new Object();
			lAnsObj.answers = [];			
			if(lAnswerOptionData.sac)
			{
				lAnsObj.sac = true;
				var lAllAnswers = lAnswerOptionData.allAnswers;
				var lAllAnswerImagePaths = lAnswerOptionData.allImageClassNames;
				for(var lAns = 0; lAns < lAllAnswers.length; ++lAns)
				{
					var lAnswerTextObj = new Object();
					lAnswerTextObj.text = lAllAnswers[lAns];
					lAnswerTextObj.ip = lAllAnswerImagePaths[lAns];
					lAnswerTextObj.ic = false;
					for(var corrAns = 0; corrAns < lCorrectAnswers.length; ++corrAns)
					{						
						if(lAllAnswers[lAns] == lCorrectAnswers[corrAns])
						{
							lAnswerTextObj.ic = true;
							break;
						}						
					}
					lAnsObj.answers[lAns] = lAnswerTextObj;
				}				
			}
			else
			{
				...
				{
					var lAnswerTextObj = new Object();
					lAnswerTextObj.text = lCorrectAnswers[corrAns];
					lAnswerTextObj.ic = true;
					lAnsObj.answers[corrAns] = lAnswerTextObj;
				}
			}
			lAnswerOptionsMap[lAnswerOptionName] = lAnsObj;
		}
		return lAnswerOptionsMap;
	}	
	...
	cp.FIBQuestion.prototype.startQuestion = function()
	{
		...
		cp.FIBQuestion.superClass.startQuestion.call(this);
	}	
	
	cp.FIBQuestion.prototype.showReviewArea = function(divElement)
	{		
		for(var key in this.answerOptionToAnswersMap)
		{
			var lAnsObj = this.answerOptionToAnswersMap[key];
		...
		var seqReviewCloseButtonElem = document.getElementById('fibReviewCloseButton');
		seqReviewCloseButtonElem.onclick = function(e) 
										{
											currSlide.parentNode.removeChild(divElement);
											parentSlideDivElement.style.cursor = 'default';
	...
	{		
		...
		if((!lCanShuffle) && (this.answerOptions.length > 0))
			return;
		
		var lTotalAnswerOptions = this.answerOptions.length;
		for(var lAnswerOptionIndex = 0; lAnswerOptionIndex < lTotalAnswerOptions; ++lAnswerOptionIndex)
		{
			var lAnswerOptionName = this.answerOptions[lAnswerOptionIndex];
			var lAnswerOptionData = cp.model.data[lAnswerOptionName];
			
			if(!lAnswerOptionData.sac)
				continue;
			if(!lAnswerOptionData.ish)
				continue;
			if(lAnswerOptionData.isShuffled == true)
				continue;
				
			var lAnswerOptionAnswersArr = this.answerOptionToAnswersMap[lAnswerOptionName].answers;
			var i;
			var len = lAnswerOptionAnswersArr.length;
			var n;
			var temp;
			var RandomAnswers_array = new Array();
			...
			for(i = 0; i < len ; i++)
			{
				RandomAnswers_array[i] = lAnswerOptionAnswersArr[i];
			}
			...
			for(i = 0; i < len ; i++)
			{
				this.answerOptionToAnswersMap[lAnswerOptionName].answers[i] = RandomAnswers_array[i];
			}			
			 
			lAnswerOptionData.isShuffled = true;
		}		
	}
	
	cp.FIBQuestion.prototype.getSelectedText = function( iAnswerOptionName )
	{
		var lAnsObj = this.answerOptionToAnswersMap[iAnswerOptionName];
		return ( lAnsObj.selectedAns == undefined ? "" : lAnsObj.selectedAns );
	}
	
	cp.FIBQuestion.prototype.getAnswerObjects = function( iAnswerOptionName )
	{
		//this function should always return the answerObjects from answerOptionToAnswersMap
		var lAnswerOptionObj = this.answerOptionToAnswersMap[iAnswerOptionName].answers;
		return lAnswerOptionObj;
	}
	
	cp.FIBQuestion.prototype.registerFIBAnswer = function( iFIBAnswerElement )
	{
		var lAnsObj = this.answerOptionToAnswersMap[iFIBAnswerElement.id];
		lAnsObj.elem = iFIBAnswerElement;
		var lAllOptionsDrawn = true;
		for(var key in this.answerOptionToAnswersMap)
		{
			var lAnsObject = this.answerOptionToAnswersMap[key];			
			lAllOptionsDrawn = lAllOptionsDrawn && (lAnsObject.elem != undefined);
		}
		this.answerOptionsDrawn = lAllOptionsDrawn;
	}
	...
	cp.FIBQuestion.prototype.checkIfCorrect = function()
	{
		...
		for(var key in this.answerOptionToAnswersMap)
		{
			var lAnsObj = this.answerOptionToAnswersMap[key];
			var lElem = lAnsObj.elem;
			lIsCorrect = lIsCorrect && ( lElem && lElem.isCorrectlyAnswered() );
			if(!lIsCorrect)
				return false;
		}
	...
	cp.FIBQuestion.prototype.getAnswerScores = function()
	{
		...
		{			
			...
			if(lAnsObj == undefined)				//should never be true
				continue;
				
			var elemData = cp.model.data[key];
			
			var lAnsScore = new cp.AnswerScore();
			lAnsScore.m_answerID = elemData.aid.toString();
			lAnsScore.m_correctAnswer = cpJoin(elemData.correctAnswers,";");
			lAnsScore.m_chosenAnswer = lAnsObj.elem.oHandler.getText();
			lAnswerScores.push(lAnsScore);			
		}
	...
	cp.FIBQuestion.prototype.setQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
	...
	cp.FIBQuestion.prototype.restoreFromQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
	...
	cp.FIBQuestion.prototype.setAnswerOrder = function(iOrder)
	{
		if(!iOrder || iOrder.length <= 0)
			return;
		...
		for(var lAnswerOptionIndex = 0; lAnswerOptionIndex < lTotalAnswerOptions; ++lAnswerOptionIndex)
		{
			var lAnswerOptionName = this.answerOptions[lAnswerOptionIndex];
			var lAnswerOptionData = cp.model.data[lAnswerOptionName];
			var lAnsObj = this.answerOptionToAnswersMap[lAnswerOptionName];
			if(!lAnsObj.sac)
				continue;
			
			var lAnswersFromLMS = iOrderArr[lAnswerOptionIndex].split(":");
			var lAnswerTextObjects = lAnsObj.answers;
			var lNewOrderArr = [];
			for(var currAnsIndex = 0; currAnsIndex < lAnswersFromLMS.length; ++currAnsIndex)
			{
				var lCurrAnsTextFromLMS = lAnswersFromLMS[currAnsIndex];				
				for(var i = 0; i < lAnswerTextObjects.length; ++i)
				{
					var lAnswerTextObj = lAnswerTextObjects[i];
					if(lAnswerTextObj.text == lCurrAnsTextFromLMS)
					{
						lNewOrderArr.push(lAnswerTextObj);
					}
				}				
			}			
				
			this.answerOptionToAnswersMap[lAnswerOptionName].answers = lNewOrderArr.splice(0);		
		}
		
		//this.answerOptions = iOrderArr.slice(0);				
		this.m_answerOrderArray = iOrder;//iOrderArr.slice(0);
	}
	
	cp.FIBQuestion.prototype.resumeSelectedAnswers = function(iAnswerScores)
	{				
		if(iAnswerScores.length <= 0)
			return;		
		...
		{
			var lAnsText = unescape(iAnswerScores[whichAns].m_chosenAnswer);
			var lAnswerOptionName = this.answerOptions[whichAns];
			var lAnswerOptionData = cp.model.data[lAnswerOptionName];
			
			var lAnsObj = this.answerOptionToAnswersMap[lAnswerOptionName];
			this.m_selectedAnswersArr[whichAns] = lAnsText;
			lAnsObj.selectedAns = lAnsText;			
		}
	}	
	...
	cp.FIBQuestion.prototype.setSelectedAnswers = function()
	{
		...
		for(var lAnswerOptionIndex = 0; lAnswerOptionIndex < lTotalAnswerOptions; ++lAnswerOptionIndex)
		{
			...
			var lAnsObj = this.answerOptionToAnswersMap[lAnswerOptionName];
			var lElem = lAnsObj.elem;
			if(lElem && lElem.oHandler)
			{
				var lText = lElem.oHandler.getText();
				this.m_selectedAnswersArr[lAnswerOptionIndex] = lText;
				lAnsObj.selectedAns = lText;
			}
		}		
	}
	...
	cp.FIBQuestion.prototype.getCorrectAnswerAsString = function()
	{
		...
		for(var lAnswerOptionIndex = 0; lAnswerOptionIndex < lTotalAnswerOptions; ++lAnswerOptionIndex)
		{
			...
			if(lAnswerOptionIndex == 0)
				lCorrAnswersAsString += cpJoin(lAnswerOptionData.correctAnswers,':');
			else
				lCorrAnswersAsString += ";" + cpJoin(lAnswerOptionData.correctAnswers,':');
		}	
		return lCorrAnswersAsString;
	}
	...
	cp.FIBQuestion.prototype.saveAnswerOrder = function()
	{
		...
		{			
			...
			if(lAnsObj.sac)
			{
				lAnswerOrder += lAnsObj.answers[0].text;
				for(var i = 1; i < lAnsObj.answers.length; ++i)
				{
					lAnswerOrder += ":" + lAnsObj.answers[i].text;
				}
			}
			else
			{				
				lAnswerOrder += "";//lAnsObj.elem.oHandler.getText();;
			}
			lAnswerOrder += ";";
		}
		lAnswerOrder = lAnswerOrder.substr(0,lAnswerOrder.length - 1);
		this.m_answerOrderArray = lAnswerOrder;
		
		this.setSelectedAnswers();
	}	

	cp.ShortAnswerQuestion = function(questionObjName,associatedObjName)
	{
		cp.ShortAnswerQuestion.baseConstructor.call(this, questionObjName, associatedObjName);
		
		this.answerOptions = this.getAnswerOptions();
		this.m_reviewAreaId = undefined;
	}
	...
	cp.ShortAnswerQuestion.prototype.startQuestion = function()
	{
		...
		var lIsInReviewMode = this.m_quizController.GetIsInReviewMode();		
		this.answerOptionsDrawn = false;
		this.answerOptionElem = undefined;
		cp.ShortAnswerQuestion.superClass.startQuestion.call(this);
	}	
	
	cp.ShortAnswerQuestion.prototype.showCustomReviewArea = function(iReviewAreaId)
	{		
		if(this.getIsCorrect())
			return true;
		if(this.getIsIncomplete())
			return false;
		//Get the id of review area for this slide from slidedata and pick its bounds to draw the review area.
		var lSlideData = cp.model.data[this.getAssociatedObjName()];
		this.m_reviewAreaId = iReviewAreaId;
			
		var lReviewAreaCanvasData = cp.model.data[this.m_reviewAreaId];
		var lReviewAreaCanvas = document.getElementById(this.m_reviewAreaId);
		if(!lReviewAreaCanvas)
			return;
		
		var lTextAlign = lReviewAreaCanvasData.ta;
		var lBold = lReviewAreaCanvasData.B;
		var lColor = lReviewAreaCanvasData.c;
		var lFont = lReviewAreaCanvasData.f;
		var lItalic = lReviewAreaCanvasData.i;
		var lSize = lReviewAreaCanvasData.sz;
		var lUnderline = lReviewAreaCanvasData.u;
		var lShowScroll = false;
		
		var otherStyleStr = "";
		otherStyleStr += ' -webkit-appearance:none;';
		otherStyleStr += ' font-family:"' + lFont + '";';
		otherStyleStr += 'font-size:' + lSize + 'px;';
		otherStyleStr += 'color: ' + lColor + ';';
		if ( lBold )
		...
		otherStyleStr += 'resize: none;overflow:auto;';
		...
		otherStyleStr += 'border-radius:0px;padding:10px;box-sizing: border-box;';
		var topPos = lReviewAreaCanvasData.b[1];
		var inputHeight = (lReviewAreaCanvasData.b[3] - lReviewAreaCanvasData.b[1]);
		if ( cp.MSIE == cp.browser ) {
			topPos -= 1;
			inputHeight += 1;
		}
		
		var lCorrAnswerTextList = this.getExpectedCorrectAnswerIDList();
		var lCorrAnsText = cpJoin(lCorrAnswerTextList,"\n");
		var innerHTMLAnsTextList = "";
		for(var j = 0; j < lCorrAnswerTextList.length; ++j)
		{
			var lTxt = lCorrAnswerTextList[j];
			if(lTxt != "")
				innerHTMLAnsTextList += "-" + lTxt + "\n";
		}		
		
		var lReviewAreaCorrectAnswersStr = this.questionData.rtt;
		lReviewAreaCanvas.innerHTML = "<div style='left:" + lReviewAreaCanvasData.b[0] + "px;top:" + (lReviewAreaCanvasData.b[1] - lSize - 5) + "px;position:absolute;width:" + (lReviewAreaCanvasData.b[2] - lReviewAreaCanvasData.b[0]) + "px; font-family:Myriad-Pro;font-size:12px;font-weight:bold;'>" + lReviewAreaCorrectAnswersStr + "</div><textarea readonly='readonly' aria-label='" + this.accstring + "' id='" + iReviewAreaId + "_inputField' style='left:" + lReviewAreaCanvasData.b[0] + "px; top:" + lReviewAreaCanvasData.b[1] + "px; width:" + (lReviewAreaCanvasData.b[2] - lReviewAreaCanvasData.b[0]) + "px; height:" + inputHeight + "px;position:absolute;" 
			+ otherStyleStr + "'>" + innerHTMLAnsTextList + "</textarea>";
	...
	{
		return ( this.m_selectedAnswersArr[0] == undefined ? "" : this.m_selectedAnswersArr[0] );
	}
	
	cp.ShortAnswerQuestion.prototype.registerShortAnswer = function( iShortAnswerElement )
	{		
		this.answerOptionElem = iShortAnswerElement;
		this.answerOptionsDrawn = this.answerOptionElem != undefined;		
	}
	...
	cp.ShortAnswerQuestion.prototype.clearAnswers = function()
	{
		...
		if(this.answerOptionElem)
			this.answerOptionElem.clearAnswer();				
	}	
	...
	{		
		...
		if(this.answerOptionElem)
		{
			this.answerOptionElem.disableOption();
			if(this.answerOptionsDrawn == true)
				this.answerOptionElem.oHandler.disable();
		}	
	...
	cp.ShortAnswerQuestion.prototype.getAnswerScores = function()
	{
		...
		var elemData = cp.model.data[this.answerOptionElem.id];
			
		var lAnsScore = new cp.AnswerScore();
		lAnsScore.m_answerID = elemData.aid.toString();
		lAnsScore.m_correctAnswer = cpJoin(this.getExpectedCorrectAnswerIDList(),";");
		lAnsScore.m_chosenAnswer = this.answerOptionElem.oHandler.getText();
		lAnswerScores.push(lAnsScore);

		return lAnswerScores;		
	}

	cp.ShortAnswerQuestion.prototype.setQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
	...
	cp.ShortAnswerQuestion.prototype.restoreFromQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
		...
		//should never reach here for short answer question. m_answerOrderArrayAsString should always be empty.
		this.setAnswerOrder(lAnswerOrder);	
	}
	
	cp.ShortAnswerQuestion.prototype.resumeSelectedAnswers = function(iAnswerScores)
	{				
		if(iAnswerScores.length <= 0)
			return;		
		...
		{
			var lAnsText = unescape(iAnswerScores[whichAns].m_chosenAnswer);
			this.m_selectedAnswersArr.push(lAnsText);			
		}
	}	
	...
	cp.ShortAnswerQuestion.prototype.setSelectedAnswers = function()
	{
		...
		{
			var lText = this.answerOptionElem.oHandler.getText();
			this.m_selectedAnswersArr.push(lText);			
		}		
	}
	...
	cp.WidgetQuestion = function(questionObjName,associatedObjName)
	{
		cp.WidgetQuestion.baseConstructor.call(this, questionObjName, associatedObjName);
		this.m_widget = undefined;
		this.m_widget_loaded = false;
		this.m_PendingFuncs = [];
		
	}
	cp.inherits(cp.WidgetQuestion, cp.Question);
	
	cp.WidgetQuestion.prototype.callLaterAfterWidgetLoad = function(aFuncName,aFuncArgs)
	{
		var lCallLater = new  cp.WidgetCallLaterData();
		lCallLater.m_Function = aFuncName;
		lCallLater.m_Args = aFuncArgs;
		this.m_PendingFuncs.push(lCallLater);
	}
	cp.WidgetQuestion.prototype.doPostWidgetLoadedStuff = function()
	{
		//do stuffs which were not done as widget_mc were not loaded at that time
	...
	cp.WidgetQuestion.prototype.setWidget = function(aWidget)
	{
		this.m_widget = aWidget;
		this.m_widget_loaded = true;
		this.doPostWidgetLoadedStuff();
		if(!this.m_isStarted)
			this.startQuestion();
	}
	
	cp.WidgetQuestion.prototype.resetWidget = function()
	{
	...
	cp.WidgetQuestion.prototype.getIsWidgetLoaded = function()
	{
		return this.m_widget_loaded;
	}
	
	cp.WidgetQuestion.prototype.startQuestion = function()
	{
		if(!this.m_widget)
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.startQuestion();});
			return;
		}
		cp.WidgetQuestion.superClass.startQuestion.call(this);
		//Set Widget State and Enable/Disable Answers
		this.setWidgetQuestionState();
		if (this.currentAttempt == 0)
		{
			if (this.m_widget && this.m_widget.enableAnswers)
				this.m_widget.enableAnswers();
		}
		var isInReviewMode = this.m_quizController.GetIsInReviewMode();
		if (isInReviewMode)
		{
	...
	cp.WidgetQuestion.prototype.showCustomReviewArea = function(iReviewAreaId)
	{		
		if(this.getIsCorrect())
			return true;
		if(this.getIsIncomplete())
		...
		if(!this.m_widget_loaded || !this.m_widget)
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.showCustomReviewArea(iReviewAreaId);});
			return;		
		}
		...
		var innerHTMLAnsTextList = this.m_widget.CorrectString();
					
		lReviewAreaCanvas.innerHTML = "<div style='left:" + lReviewAreaCanvasData.b[0] + "px;top:" + (lReviewAreaCanvasData.b[1] - lSize - 5) + "px;position:absolute;width:" + (lReviewAreaCanvasData.b[2] - lReviewAreaCanvasData.b[0]) + "px; font-family:Myriad-Pro;font-size:12px;font-weight:bold;'>CORRECT ANSWERS</div><textarea readonly='readonly' aria-label='" + this.accstring + "' id='" + iReviewAreaId + "_inputField' style='left:" + lReviewAreaCanvasData.b[0] + "px; top:" + lReviewAreaCanvasData.b[1] + "px; width:" + (lReviewAreaCanvasData.b[2] - lReviewAreaCanvasData.b[0]) + "px; height:" + inputHeight + "px;position:absolute;" 
			+ otherStyleStr + "'>" + (innerHTMLAnsTextList?innerHTMLAnsTextList:"") + "</textarea>";
		
		return true;
	}
	
	cp.WidgetQuestion.prototype.endQuestion = function(iWasJudged)
	{
		  cp.WidgetQuestion.superClass.endQuestion.call(this,iWasJudged);		  
	}
	
	cp.WidgetQuestion.prototype.clearAnswers = function()
	{
		  if(!this.m_widget_loaded || !this.m_widget)
			return;
		  //WIDGET FUNCTION
		  if(this.m_widget.clearAnswers)
			this.m_widget.clearAnswers();
	}
	
	cp.WidgetQuestion.prototype.enableAnswers = function()
	{
		  if(!this.m_widget_loaded || !this.m_widget)
			return;
		  //WIDGET FUNCTION
		  if(this.m_widget.enableAnswers)
			this.m_widget.enableAnswers();
	}
	
	cp.WidgetQuestion.prototype.disableAnswers = function()
	{
		  if(!this.m_widget_loaded || !this.m_widget)
			return;
		  //WIDGET FUNCTION
		  if(this.m_widget.disableAnswers)
			this.m_widget.disableAnswers();
	}	
	
	cp.WidgetQuestion.prototype.setAnswersAccessibility = function()
	{
		 if(!this.m_widget_loaded || !this.m_widget)
			return;
		  //WIDGET FUNCTION
		  if(this.m_widget.setAccessibility)
			this.m_widget.setAccessibility();
	}
	
	cp.WidgetQuestion.prototype.checkAndSetQuestionStatus = function()
	{
		var questionStatus = this.QuestionStatusEnum.INCOMPLETE;
		
		if(!this.m_widget_loaded || !this.m_widget || !this.m_quizController)
			return questionStatus;
		  //WIDGET FUNCTION
		var lAnsweredCorrectly = false;
		var lAnswerIncomplete = false;
		//WIDGET FUNCTION
		if(this.m_widget.answersIncomplete)
		{
			var lMustAnswer = this.m_quizController.GetQuestionAdvanceType() == 'mustAnswer';
			lAnswerIncomplete =  this.m_widget.answersIncomplete(lMustAnswer);
			if(lAnswerIncomplete)
				 questionStatus = this.QuestionStatusEnum.INCOMPLETE;
		}
		
		if(!lAnswerIncomplete && this.m_widget.answeredCorrectly)
		{
			lAnsweredCorrectly = this.m_widget.answeredCorrectly(this.getIsSurvey());
			questionStatus = lAnsweredCorrectly?this.QuestionStatusEnum.CORRECT:this.QuestionStatusEnum.INCORRECT;
		}		
		
		this.setQuestionStatus(questionStatus);
	}
	
	cp.WidgetQuestion.prototype.setWidgetQuestionState = function()
	{
		if(!this.m_questionScore)
			return;
		if(!this.m_widget_loaded || !this.m_widget)
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.setWidgetQuestionState();});
			return;		
		}
		if(this.m_widget.setWidgetQuestionState)
			this.m_widget.setWidgetQuestionState( this.m_selectedAnswersArr.toString() );
	}

	cp.WidgetQuestion.prototype.saveAnswerOrder = function()
	{		
		if(!this.m_questionScore)
			return;
		if(!this.m_widget_loaded || !this.m_widget)
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.saveAnswerOrder();});
			return;		
		}
		if(this.m_widget.getWidgetQuestionState)
			this.m_selectedAnswersArr = this.m_widget.getWidgetQuestionState().split(",");		
	}
	
	cp.WidgetQuestion.prototype.GetAnswers = function() //Return Widget Answers Array
	{
		var lAnswers = [];
		if(!this.m_widget_loaded || !this.m_widget || !this.m_widget.answers)
			return lAnswers;
			
		var lWidgetAnswers = this.m_widget.answers();
		if(!lWidgetAnswers)
			return lAnswers;
		
		for (var i=0; i<lWidgetAnswers.length; ++i)
			lAnswers.push(lWidgetAnswers[i]);
			
		return lAnswers;
	}
	
	cp.WidgetQuestion.prototype.getAnswerScores = function()
	{
		var lAnswerScores = [];				
		var lAnswers = this.GetAnswers();		
		for (var ans in lAnswers) 
		{
			var lAnsScore = new cp.AnswerScore();
			lAnsScore.m_answerID = lAnswers[ans].answerID;			
			lAnsScore.m_chosenAnswer = lAnswers[ans].chosenAnswer;
			lAnsScore.m_correctAnswer = lAnswers[ans].correctAnswer;			
			if ((lAnsScore.m_answerID != undefined) && (lAnsScore.m_answerID.length > 0)) 
			{
				lAnswerScores.push(lAnsScore);	
			}
		}
	...
	cp.WidgetQuestion.prototype.disableAllOptions = function()
	{
		if(!this.m_widget_loaded || !this.m_widget)
			return;
	...
	cp.WidgetQuestion.prototype.setQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
		...
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.setQuestionSpecificScoreProperties(aQuestionSpecificScore);});
			return;		
		}
		if(this.m_widget.getWidgetQuestionState)
			aQuestionSpecificScore.m_answerOrderArrayAsString = this.m_widget.getWidgetQuestionState();
	}

	cp.WidgetQuestion.prototype.restoreFromQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
		...
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.restoreFromQuestionSpecificScoreProperties(aQuestionSpecificScore);});
			return;		
		}
		if(this.m_widget.setWidgetQuestionState)		
			this.m_widget.setWidgetQuestionState(aQuestionSpecificScore.m_answerOrderArrayAsString);
	}
	
	cp.WidgetQuestion.prototype.getChosenAnswerAsString = function()
	{
		if(this.getAnsweredCorrectly())
			return "1";
		else
	...
	cp.WidgetQuestion.prototype.getCorrectAnswerAsString = function()
	{
		return "1";
	}
	
	cp.InteractionCallLaterData = function()
	{
		this.m_Function; 
		this.m_Args = [];
	}
	
	cp.InteractiveItemQuestion = function(questionObjName,associatedObjName)
	{
		cp.InteractiveItemQuestion.baseConstructor.call(this, questionObjName, associatedObjName);
		this.m_item = undefined;	
	}
	...
	cp.InteractiveItemQuestion.prototype.endQuestion = function(iWasJudged)
	{
		if((this.state == this.StateEnum.PLAYING) || (this.state == this.StateEnum.PAUSED))
		{
			var lIsInReviewMode = this.m_quizController && this.m_quizController.GetIsInReviewMode();				
		...
		cp.InteractiveItemQuestion.superClass.endQuestion.call(this,iWasJudged);		  
	}
	
	cp.InteractiveItemQuestion.prototype.enable = function()
	{
		//set the enabled state of item here
	}
	
	cp.InteractiveItemQuestion.prototype.disable = function()
	{
		//set the disabled state of item here
	}
	...
	cp.InteractiveItemQuestion.prototype.setInteractionQuestionState = function()
	{
		if(!this.m_questionScore)
			return;
		//set the last state of interactive item here
	}
	...
	{		
		...
		this.m_answerOrderArray = [];
		//save the current state of the interactive item here
	}
	
	cp.InteractiveItemQuestion.prototype.disableAnswers = function()
	{
		//disable the interactive item here
	}
	...
	cp.InteractiveItemQuestion.prototype.setQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
		
		aQuestionSpecificScore.m_answerOrderArrayAsString = "";
		//Set the answer order string from the current state of interactive item
	}

	cp.InteractiveItemQuestion.prototype.restoreFromQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
		
		//Restore the interactive item state from the aQuestionSpecificScore.m_answerOrderArrayAsString value
	}
	
	cp.InteractiveWidgetQuestion = function(questionObjName,associatedObjName)
	{
		cp.InteractiveWidgetQuestion.baseConstructor.call(this, questionObjName, associatedObjName);
		this.m_widget = undefined;
		this.m_widget_loaded = false;
		this.m_PendingFuncs = [];
		
	}
	cp.inherits(cp.InteractiveWidgetQuestion, cp.InteractiveItemQuestion);
	
	cp.InteractiveWidgetQuestion.prototype.getIsInteractiveWidget = function()
	{
		return true;
	}
	
    cp.InteractiveWidgetQuestion.prototype.resetQuestionData = function()
	{
		cp.InteractiveWidgetQuestion.superClass.resetQuestionData.call(this);
		if(!this.m_quizController.GetGoToQuizScopeActionExecuted())
		{
			this.m_selectedAnswersArr = [];
			this.m_answerOrderArray = [];
	...
	cp.InteractiveWidgetQuestion.prototype.getInteractionType = function()
	{
		if(!this.m_questionScore)
			return;
		if(!this.m_widget_loaded || !this.m_widget)
		{
			var that = this;
			this.callLaterAfterWidgetLoad(function (){ that.getInteractionType();});			
			return;		
		}
		this.interactionType = cp.InteractiveWidgetQuestion.superClass.getInteractionType.call(this);
		if(this.m_widget.getInteractionType)
			this.interactionType = this.m_widget.getInteractionType();		
		
		return this.interactionType;
	}
	
	cp.InteractiveWidgetQuestion.prototype.callLaterAfterWidgetLoad = function(aFuncName,aFuncArgs)
	{
		var lCallLater = new  cp.WidgetCallLaterData();
		lCallLater.m_Function = aFuncName;
		lCallLater.m_Args = aFuncArgs;
	...
	cp.InteractiveWidgetQuestion.prototype.doPostWidgetLoadedStuff = function()
	{
		//do stuffs which were not done as widget_mc were not loaded at that time
		var lNumFunctions = this.m_PendingFuncs.length
		for (var i=0; i<lNumFunctions; ++i)
	...
	cp.InteractiveWidgetQuestion.prototype.setWidget = function(aWidget)
	{
		this.m_widget = aWidget;
		this.m_widget_loaded = true;
		this.doPostWidgetLoadedStuff();
	...
	cp.InteractiveWidgetQuestion.prototype.resetWidget = function()
	{
        this.endQuestion(false);
        this.doPostWidgetLoadedStuff();
		this.m_widget = undefined;
	...
	cp.InteractiveWidgetQuestion.prototype.getIsWidgetLoaded = function()
	{
		return this.m_widget_loaded;
	}
	
	cp.InteractiveWidgetQuestion.prototype.startQuestion = function()
	{
		if(!this.m_widget)
		{
			var that = this;
		...
		cp.InteractiveWidgetQuestion.superClass.startQuestion.call(this);		
	}
	
	cp.InteractiveWidgetQuestion.prototype.endQuestion = function(iWasJudged)
	{
		  cp.InteractiveWidgetQuestion.superClass.endQuestion.call(this,iWasJudged);		  
	}
	
	cp.InteractiveWidgetQuestion.prototype.enable = function()
	{
		if (this.m_widget && this.m_widget.enable)
		{			
			this.m_widget.enable();
		}
	}
	
	cp.InteractiveWidgetQuestion.prototype.disable = function()
	{		
		if (this.m_widget && this.m_widget.disable)
		{
			this.m_widget.disable();
		}
	}
	
	cp.InteractiveWidgetQuestion.prototype.enableAnswers = function()
	{
		this.isDisabled = false;
		  if(!this.m_widget_loaded || !this.m_widget)
			return;
		  //WIDGET FUNCTION
		  if(this.m_widget.enable)
			this.m_widget.enable();
	}
	
	cp.InteractiveWidgetQuestion.prototype.disableAnswers = function()
	{
		this.isDisabled = true;
		  if(!this.m_widget_loaded || !this.m_widget)
			return;
		  //WIDGET FUNCTION
		  if(this.m_widget.disable)
			this.m_widget.disable();
	}	
	
	cp.InteractiveWidgetQuestion.prototype.setInteractionQuestionState = function()
	{
		if(!this.m_questionScore)
			return;
		if(!this.m_widget_loaded || !this.m_widget)
		...
		if(this.m_widget.setInteractionQuestionState)
			this.m_widget.setInteractionQuestionState( this.m_answerOrderArray.toString() );
	}

	cp.InteractiveWidgetQuestion.prototype.saveAnswerOrder = function()
	{		
		if(!this.m_questionScore)
			return;
		if(!this.m_widget_loaded || !this.m_widget)
		...
		if(this.m_widget.getInteractionQuestionState)
			this.m_answerOrderArray = this.m_widget.getInteractionQuestionState().split(",");		
	}
	
	cp.InteractiveWidgetQuestion.prototype.setQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
	...
	cp.InteractiveWidgetQuestion.prototype.restoreFromQuestionSpecificScoreProperties = function(aQuestionSpecificScore)
	{
		if(aQuestionSpecificScore == undefined)
			return;
        ...
        this.m_answerOrderArray = aQuestionSpecificScore.m_answerOrderArrayAsString.split(',');
		this.setInteractionQuestionState();
	}
	...
	cp.DisplayObject.prototype =
	{
		...
		drawComplete: function()
		{
			if(!this.isRegistered)
			{
				...
				{
					currInteraction.registerDisplayObject(this.element.id);
				}
			}
		},
	...
	cp.QuestionSlideReviewLabel.prototype.addIfNeeded = function()
	{
		...
		{
			this.isDrawn = lQuestionObj.showCustomReviewArea(this.element.id);
			return;
		}
	...
	cp.ProgressSlideLabel.prototype.addIfNeeded = function()
	{
		...
		if(this.element.innerHTML == "")
		{
			this.element.innerHTML += "<table style='width=100%;height:100%;'><tr style='width=100%;height:100%;'><td  style='width=100%;height:100%;vertical-align:middle;'>" + this.text + "</td></tr></table>";	
		}
    ...
    cp.ResultSlideLabel = function(el, args)
    {
		...
		{
			//THIS IS QUIZ RESULT FEEDBACK MESSAGE
			this.textAlign = "left";//THIS IS DONE FOR FIX OF BUG#3303148. MAKING IT LEFT ALIGNED FORCEFULLY.
	...
	cp.AnimationItem = function(el, args)
    {
		...
		if(this.element)
		{
			...
			{
				this.actualParent.drawingBoard = this.element;
			}
	...
	//this should act as parent class for all the quiz slide items - 
	//1. radio buttons
	//2. check boxes
	//3. sequence inputs
	//4. matching items and matching answers
	//5. hotspot inputs
	cp.Answer = function(el,args)
	{
		cp.Answer.baseConstructor.call(this, el);
	...
	cp.Answer.prototype.start = function()
	{
		this.addIfNeeded();
	}

	cp.Answer.prototype.reset = function(endOfSlide)
	{
		// release memory
		delete ropMap[this.element.id];
		this.isDrawn = false;
	...
	{
		//TODO
		//common code will be moved here
	}	

	// HotspotInput CLASS for Hotspot Input
	...
	{	
		...
		lQuestionObj.registerHotspotInput(this);
	}
	...
	cp.CheckBoxInput.prototype.addIfNeeded = function()
	{
		...
		parentSlideElement.insertBefore(dummyHighlight,answerLabelCanvasElement.parentElement);
		...
		parentSlideElement.insertBefore(answertextParentDiv,answerLabelCanvasElement.parentElement);
		...
		var answerLabelCanvasParentSiblingElement = answerLabelCanvasParentElement.nextSibling;
		parentSlideElement.removeChild(answerLabelCanvasParentElement);
		parentSlideElement.insertBefore(answerLabelCanvasParentElement,answerLabelCanvasParentSiblingElement);
		...
		parentSlideElement.insertBefore(newPa,answerLabelCanvasParentSiblingElement);
	...
	cp.SequenceInput.prototype.addIfNeeded = function()
	{
		...
		var lAnswerLabelCanvasData = cp.model.data[divData.cn];
		this.lAnswerLabelCanvasElem = document.getElementById(lAnswerLabelCanvasData.dn);
		...
		if(lShouldDisable)
		{
			...
			{				
				...
				if(lQuestionObj.getAnsweredCorrectly())
				{
					...
					this.lAnswerLabelCanvasElem.appendChild(newImg);
					if(self.sh)
					{
						var mAngle = self.sh.a;
	...
	cp.MatchingItem = function(el, args)
	{
		cp.MatchingItem.baseConstructor.call(this,el, args);
	...
	cp.MatchingItem.prototype.addAndGetLabelCombo = function(lQuestionObj,iDivData,iSelectedIndex)
	{		
		this.answerLabelCanvasNames = lQuestionObj.getAnswerLabels();
		var firstAnswerLabelCanvasDivData = cp.model.data[this.answerLabelCanvasNames[0]];
		var answerLabelCanvasImageBounds = (firstAnswerLabelCanvasDivData['b']);
		if (answerLabelCanvasImageBounds.length == 4)
		{
			for(var i=0; i<4; i++)
				answerLabelCanvasImageBounds[i] = parseFloat(answerLabelCanvasImageBounds[i]);
		}
		else 
		{
			answerLabelCanvasImageBounds = [0, 0, 0, 0];
		}
			
		this.answerLabelCanvasImageBounds = {
				minX: answerLabelCanvasImageBounds[0],
				minY: answerLabelCanvasImageBounds[1],			
				maxX: answerLabelCanvasImageBounds[2],
				maxY: answerLabelCanvasImageBounds[3]
			};					
		
		var lAnswerLabelCanvasImageWidth = (this.answerLabelCanvasImageBounds.maxX - this.answerLabelCanvasImageBounds.minX);
		var lAnswerLabelCanvasImageHeight = (this.answerLabelCanvasImageBounds.maxY - this.answerLabelCanvasImageBounds.minY);

		var lAnswerHolderElem = document.getElementById(iDivData.answerTextCanvasHolder);

		var labelDropDownElement = document.createElement("div");
		labelDropDownElement.id = this.element.id + "_labelDD";	
		labelDropDownElement.style.position = 'absolute';		
		labelDropDownElement.style.left = "0px";
		labelDropDownElement.style.top = "-2px";
		labelDropDownElement.style.width = lAnswerLabelCanvasImageWidth + "px";
		labelDropDownElement.style.height = lAnswerLabelCanvasImageHeight + "px";
				
		lAnswerHolderElem.appendChild(labelDropDownElement);
				
		if(labelDropDownElement.innerHTML == "")
		{						
			var innerHTMLStr = "";
			innerHTMLStr += "<select class='mydds' id='" + this.element.id + "_matchingLabel' name='" + this.group + "'>";
			for(var z = 0; z < this.answerLabelCanvasNames.length; ++z)
			{
				var answerLabelCanvasDivData = cp.model.data[this.answerLabelCanvasNames[z]];
				var answerLabelCanvasImageSrc = answerLabelCanvasDivData.lip;
											
				innerHTMLStr += "<option value='" + this.answerLabelCanvasNames[z] + "' style='position:absolute;width:" + (lAnswerLabelCanvasImageWidth) + "px' title='" + answerLabelCanvasImageSrc + "'>"
				
				if(cp.DESKTOP != cp.device)
				{					
					innerHTMLStr += answerLabelCanvasDivData.accstr;
				}
			...
			document.getElementById(labelDropDownElement.id).innerHTML = innerHTMLStr;
			var selectElement = document.getElementById(this.element.id + "_matchingLabel");
			
			selectElement.style.width = (lAnswerLabelCanvasImageWidth + 30) + "px";
			selectElement.style.height = (lAnswerLabelCanvasImageHeight) + "px";
			selectElement.style.position = 'absolute';
			...
			{		
				...
				selectElement.onchange = function(e)
										{
											self.element.oHandler.setIndex(selectElement.selectedIndex);												
										};
				
				var lTouchedCtr = 0;				
				
				var spaninnerStr = "<img id='" + this.element.id + "_spanImage' style='position:absolute;top:2px; left:5px; width:" + lAnswerLabelCanvasImageWidth + "px; height:" + (lAnswerLabelCanvasImageHeight) + "px;'/>";
				spaninnerStr += "<img id='" + this.element.id + "_spanImageArrow' class='spanArrow' style='position:absolute;left:" + (lAnswerLabelCanvasImageWidth + 5) + "px;top:3px'/>";
				
				var spanChild = document.createElement('span');
				spanChild.id='spanComboBoxTitleImage';
				spanChild.className = 'spanComboBox';
				spanChild.innerHTML = spaninnerStr;
				spanChild.style.width = (lAnswerLabelCanvasImageWidth + 30) + "px";
				spanChild.style.height = (lAnswerLabelCanvasImageHeight + 3) + "px";
				
				this.answerLabelDivElem.appendChild(spanChild);	
				
				var lTitleSpanElem = document.getElementById(this.element.id + "_spanImage");					
				selectElement.setNewIndex = function(iIndex)
				{										
					if(iIndex != -1)
					{							
						self.answerLabelCanvasName = self.answerLabelCanvasNames[iIndex];
						var lSelectedImageData = cp.model.data[self.answerLabelCanvasName];
						lTitleImageSrc = lSelectedImageData.ip;						
						selectElement.selectedIndex = iIndex;						
						self.element.drawLine(self.questionObj.getXYForColumn2Option(lSelectedImageData.aid,self.element));
					}
					else
					{
						//cp.log("reached here inside else");
						if(cp.verbose)
						{
							cp.log("Inside setNewIndex : " + iIndex);							
						}						
						self.answerLabelCanvasName = "";
						lTitleImageSrc = 'assets/htmlimages/placeholder.png';
						selectElement.selectedIndex = -1;
						selectElement.value = "";
						selectElement.text = "";
					}
				...
				this.answerLabelDivElem.style.left = answerLabelCanvasImageBounds.minX + "px";
				this.answerLabelDivElem.style.top = answerLabelCanvasImageBounds.minY + "px";
				this.answerLabelDivElem.style.width = (lAnswerLabelCanvasImageWidth) + "px";
				this.answerLabelDivElem.style.height = lAnswerLabelCanvasImageHeight + "px";
				...
				selectElement.resetIndex = function()
				{
					self.element.oHandler.setIndex(-1);
				};
				
				selectElement.setIndex = function(iIndex)
				{					
					self.element.oHandler.setNewIndex(iIndex);
				};

				selectElement.getIndex = function()
				{					
					return self.element.oHandler.selectedIndex;
				};

				selectElement.disable = function()
				{
					self.element.oHandler.disabled = 'disabled';
				};

				selectElement.getWidth = function()
				{
					return parseFloat(self.element.oHandler.style.width);
				};

				oHandler = selectElement;
			}
			else
			{
				var lLeftPosition = parseFloat(selectElement.style.left) + (this.answerLabelCanvasImageBounds.minX);
				var lTopPosition = parseFloat(selectElement.style.top) + (this.answerLabelCanvasImageBounds.minY);
				
				selectElement.style.position = 'absolute';
				selectElement.style.left = answerLabelCanvasImageBounds.minX + "px";
				selectElement.style.top = answerLabelCanvasImageBounds.minY + "px";
				selectElement.style.width = (lAnswerLabelCanvasImageWidth + 30) + "px";
				selectElement.style.height = lAnswerLabelCanvasImageHeight + "px";
				
				var lSelectElement = jQuery("#" + this.element.id + "_matchingLabel");
				lSelectElement.css("width",(lAnswerLabelCanvasImageWidth + 30));				
				oHandler = jQuery("#" + this.element.id + "_matchingLabel").msDropDown({style:"position:fixed, z-index:1000;left:" + (answerLabelCanvasImageBounds.minX + 20) + "px, top:" + (answerLabelCanvasImageBounds.minY) + "px, height:" + (lAnswerLabelCanvasImageHeight) + "px, width:" + (lAnswerLabelCanvasImageWidth) + "px,"},"").data("dd");	
				oHandler.set("width",(lAnswerLabelCanvasImageWidth + 30));
				var self = this;
				var lDrawLine = function(iIndex)
								{
									if(iIndex != -1)
									{
										self.answerLabelCanvasName = self.answerLabelCanvasNames[iIndex];
				...
				var onClose = function()
										{
											var lIndex = self.element.oHandler.get("selectedIndex");
											lDrawLine(lIndex);
										};
				
				oHandler.addMyEvent("onClose",onClose);
				...
				this.answerLabelDivElem.onclick = function(e)
				{											
					self.element.oHandler.open();//THIS IS TO BE LOOKED ONCE AGAIN											
				};	
				
				oHandler.resetIndex = function()
				{
					self.element.oHandler.selectedIndex(-1);
				};
				
				oHandler.setIndex = function(iIndex)
				{
					self.element.oHandler.selectedIndex(iIndex);
					lDrawLine(iIndex);
					//drawLineOnClose();
				};

				oHandler.getIndex = function()
				{					
					return self.element.oHandler.get("selectedIndex");
				};

				oHandler.disable = function()
				{
					self.element.oHandler.disabled(true);
				};

				oHandler.getWidth = function()
				{
					return self.element.oHandler.get("width");
				};
			}		
		}
	...
	cp.MatchingItem.prototype.addIfNeeded = function()
	{		
		var lQuestionObj = getQuestionObject(this.relatedQuestionSlide);
		if(!lQuestionObj)
			return;
			
		this.questionObj = lQuestionObj;
		if (this.isDrawn || !lQuestionObj.getIsStarted())
			return;
		...
		var lCurrentOptionObj = lQuestionObj.getAnswerOption(id);
		this.answerTextCanvasDivName = lCurrentOptionObj.name;
		var answerTextCanvasDivData = cp.model.data[this.answerTextCanvasDivName];
		...
		var self = this;
		this.element.drawLine = function(iPos)
		{			
			if(iPos == undefined)
				return;
			var startX = (parseFloat(self.element.parentElement.style.left) + parseFloat(self.element.parentElement.style.width));// + "px";
			var startY = parseFloat(self.element.parentElement.style.top) + parseFloat(self.element.parentElement.style.height)/2;// + "px";
			...
			if(cp.verbose)
			{
				cp.log("startX : " + startX);
				cp.log("startY : " + startY);
				cp.log("endX : " + endX);
				cp.log("endY : " + endY);
			}
			
			var lL = (startX < endX ? startX : endX);
			var lT = startY < endY ? startY : endY;
			var lW = (( endX - startX ) > 0 ? ( endX - startX ) : ( startX - endX ));
			var lH = ( endY - startY ) > 0 ? ( endY - startY ) : ( startY - endY );
			
			self.lLineCanvasID = (self.element.id + startX.toString() + startY.toString());

			var lLineCanvasDiv = document.getElementById(self.lLineCanvasID  + "_matchingLineCanvasDiv");
			if(lLineCanvasDiv == undefined)
			{
				lLineCanvasDiv = document.createElement('div');
				lLineCanvasDiv.id = self.lLineCanvasID  + "_matchingLineCanvasDiv";
			}
			else
			{
				lLineCanvasDiv.innerHTML = "";
			}
			
			lLineCanvasDiv.style.display = "block";
			lLineCanvasDiv.style.position = "absolute";
			lLineCanvasDiv.style.left = (lL - 10) + "px";
			lLineCanvasDiv.style.top = (lT - 10) + "px";
			lLineCanvasDiv.style.width = (lW + 20) + "px";
			lLineCanvasDiv.style.height = (lH + 20) + "px";
			
			cp.addRewrapObjectAsPerRestOfProjectItem(lLineCanvasDiv);	
			
			var lLineCanvas = document.createElement('canvas');
			lLineCanvas.id = self.lLineCanvasID + "_matchingLineCanvas";
			lLineCanvasDiv.appendChild(lLineCanvas);
						
			lLineCanvas.width = lW + 20;
			lLineCanvas.height = lH + 20;					
			
			var gc = lLineCanvas.getContext("2d");
			gc.beginPath();
			gc.moveTo( startX - parseFloat(lLineCanvasDiv.style.left), startY - parseFloat(lLineCanvasDiv.style.top));
			gc.lineTo( endX - parseFloat(lLineCanvasDiv.style.left), endY - parseFloat(lLineCanvasDiv.style.top));
			gc.stroke();
			gc = null;
			
			lLineCanvas.style.display = "block";
		};	
		...
		this.element.resetMatch = function()
		{
			...
			lLineCanvasDiv.parentElement.removeChild(lLineCanvasDiv);
			self.element.oHandler.resetIndex();
		}
		
		this.element.removeOpenReviewAreas = function()
		{
			var currSlide = document.getElementById(self.questionObj.getAssociatedObjName());
			var reviewAreaDiv = document.getElementById(self.element.id + "_correctMatchingReview");
			if(reviewAreaDiv != undefined)
				currSlide.parentNode.removeChild(reviewAreaDiv);
		};
		
		this.element.addReviewButton = function()
		{
			var currSlide = document.getElementById(self.questionObj.getAssociatedObjName());
			var lOptionElem = self.element;
			var lReviewButtonId = self.element.id + "_reviewButton";
			var lReviewButton = document.getElementById(lReviewButtonId);
			if(!lReviewButton)
				lReviewButton = document.createElement('img');
			lReviewButton = lReviewButton;
			...
			lReviewButton.className = 'sequenceReviewImage';						
			lReviewButton.style.position = 'absolute';
			lReviewButton.style.left = (parseFloat(lOptionElem.parentElement.style.left) + parseFloat(lOptionElem.style.left))+ "px";
			lReviewButton.style.top = (parseFloat(lOptionElem.parentElement.style.top) + parseFloat(lOptionElem.parentElement.style.height)) + "px";			
			lReviewButton.onclick = function(e)
							{	
								var lCorrectMatchingDivId = lOptionElem.id + "_correctMatchingReview";
								var correctMatchingDiv = document.getElementById(lCorrectMatchingDivId);
								if(correctMatchingDiv == undefined)
									correctMatchingDiv = document.createElement('div');
									
								var innerHTMLStr = "<ul style='padding-left:20px;padding-right:20px'>";
																
								for(var i = 0; i < self.questionObj.Column2Options.length; ++i)
								{
									var currOptionData = cp.model.data[self.questionObj.Column2Options[i]];
									var lOptionElemData = cp.model.data[lOptionElem.id];
									var lOptionElemCanvasData = cp.model.data[lOptionElemData.answerTextDivName];
									if(currOptionData.aid == lOptionElemCanvasData.aid)
										innerHTMLStr += "<li>" + currOptionData['atxtlms'] + "</li>";
								}
								
								innerHTMLStr += "</ul>"
								innerHTMLStr += "<img id='matchingReviewCloseButton' src='./assets/htmlimages/closeReviewButton.png' style='right:6px;top:6px;position:absolute'>";
								correctMatchingDiv.id = lCorrectMatchingDivId;
								correctMatchingDiv.className = 'sequenceReviewArea';
								correctMatchingDiv.style.position = 'absolute';
								
								var lSelfReviewButton = document.getElementById(lOptionElem.id + "_reviewButton");
								correctMatchingDiv.style.left = (parseFloat(lSelfReviewButton.style.left) + parseFloat(lSelfReviewButton.width) + 10) + "px";
								correctMatchingDiv.style.top = 	parseFloat(lSelfReviewButton.style.top) + "px";
								correctMatchingDiv.innerHTML = innerHTMLStr;
								
								if (e.stopPropagation) 
								{
									e.stopPropagation(); // stops the browser from redirecting.
								}
								self.questionObj.showReviewArea(correctMatchingDiv);								
							};
		...
		var actualID = divData.actid;
		var answertextParentDiv = document.createElement('div');			
		//parentSlideElement.insertBefore(answertextParentDiv,answerLabelCanvasElement.parentElement);
		cp.addRewrapObjectAsPerRestOfProjectItem(answertextParentDiv);
		answertextParentDiv.id = this.element.id + "canvasHolder";
		divData["answerTextCanvasHolder"] = answertextParentDiv.id;
		answertextParentDiv.setAttribute('class','cp-rewrap');
		
		var matchingAnswerCanvasElement = document.createElement('canvas');
		canvas = this.canvas = cp.createCanvas(0, 0,  cp.model.data.project.w, cp.model.data.project.h,matchingAnswerCanvasElement);
		var gc 	= canvas.gc;
		
		matchingAnswerCanvasElement.setAttribute('class','cp-shape');			
		matchingAnswerCanvasElement.setAttribute('name',this.group);		
		matchingAnswerCanvasElement.id = this.answerTextCanvasDivName;

		answertextParentDiv.appendChild(matchingAnswerCanvasElement);
		var answertextCanvasShape = new cp.Shape(matchingAnswerCanvasElement, cp.model.data[this.answerTextCanvasDivName]);
		answertextCanvasShape.start();

		//add label combo-box here
		var answerLabelCanvasParentElement = answerLabelCanvasElement.parentElement;
		parentSlideElement.removeChild(answerLabelCanvasParentElement);
		
		var lAnswerLabelCanvasData = cp.model.data[this.labelCanvasDivName];
		var lAnswerLabelDivElemName = lAnswerLabelCanvasData.dn;		
		this.answerLabelDivElem = document.getElementById(lAnswerLabelDivElemName);	
		
		var selectedIndex = lQuestionObj.getSelectedIndex(lCurrentOptionObj.selectedAnswerLabel);
		this.element.oHandler = this.addAndGetLabelCombo(lQuestionObj,divData,selectedIndex);
		divData["oHandler"] = this.element.oHandler;
		
		this.element.oHandler.setIndex(selectedIndex);
		//added label combo-box

		var lActualIDDivData = cp.model.data[actualID];
		var lCanvasName = lActualIDDivData.mdi;
		
		var lLeftPosition = parseFloat(this.element.oHandler.getWidth()) + 10;
		var lTopPosition = parseFloat(answertextParentDiv.style.top) + (cp.model.data[lCanvasName].b[1] - this.answerTextCanvasImageBounds.minY);
					
		matchingAnswerCanvasElement.style.left = lLeftPosition + 'px';
		matchingAnswerCanvasElement.parentElement.style.top = lTopPosition + 'px';
		
		answertextParentDiv.style.width = (parseFloat(answertextParentDiv.style.width) + lLeftPosition) + "px";		
		answertextParentDiv.style.left = lAnswerLabelCanvasData.b[0] + "px";				
		...
		if(!lShouldDisable)
			lQuestionObj.addDragEvents(this.element.id);		
		
		lReviewIconPositionLeft = (parseFloat(this.element.style.left) - parseFloat(answertextParentDiv.style.left) - 30) + "px";
		lReviewIconPositionTop = "0px";
		
		this.isCorrect = lQuestionObj.getIsOptionCorrect(this.element,selectedIndex);		

		if(lShouldDisable)
		{
			this.element.style.cursor = "default";
			this.element.oHandler.disable();
			this.element.tabIndex = -1;
			if(lQuestionObj.getWasJudged() &&  lQuestionObj.canShowReviewIcons())
			{
				if(lQuestionObj.getAnsweredCorrectly())
		...
		lQuestionObj.registerColumn1Option(this.element);
				
		if(!this.visible)
			this.element.style.visibility = 'hidden';
	}
	
	cp.MatchingAnswer = function(el, args)
	{
		cp.MatchingAnswer.baseConstructor.call(this,el, args);
	...
	cp.MatchingAnswer.prototype.addIfNeeded = function()
	{
		var lQuestionObj = getQuestionObject(this.relatedQuestionSlide);
		if(!lQuestionObj)
			return;
		...
		this.answerTextCanvasDivName = cp.model.data[divData.actid].mdi;//lQuestionObj.getAnswerOption(this.answerID,id);
		var answerTextCanvasDivData = cp.model.data[this.answerTextCanvasDivName];
		...
		matchingAnswerCanvasElement.setAttribute('class','cp-shape');			
		matchingAnswerCanvasElement.setAttribute('name',this.group);
		//matchingAnswerCanvasElement.setAttribute('draggable','true');
		matchingAnswerCanvasElement.id = this.answerTextCanvasDivName;
		answertextParentDiv.appendChild(matchingAnswerCanvasElement);
		var answertextCanvasShape = new cp.Shape(matchingAnswerCanvasElement, cp.model.data[this.answerTextCanvasDivName]);
		answertextCanvasShape.start();					
		...
		var lLeftPosition = parseFloat(matchingAnswerCanvasElement.parentElement.style.left) + (cp.model.data[lCanvasName].b[0] - this.answerTextCanvasImageBounds.minX);
		var lTopPosition = parseFloat(matchingAnswerCanvasElement.parentElement.style.top) + (cp.model.data[lCanvasName].b[1] - this.answerTextCanvasImageBounds.minY);
					
		matchingAnswerCanvasElement.parentElement.style.left = lLeftPosition + 'px';
		matchingAnswerCanvasElement.parentElement.style.top = lTopPosition + 'px';
				
		var answerLabelCanvasParentElement = answerLabelCanvasElement.parentElement;		
		var lLabelCanvasLeftPosition = lLeftPosition - parseFloat(answerLabelCanvasParentElement.style.left);
		var lLabelCanvasTopPosition = lTopPosition - parseFloat(answerLabelCanvasParentElement.style.top);
		parentSlideElement.removeChild(answerLabelCanvasParentElement);
		answertextParentDiv.appendChild(answerLabelCanvasParentElement);
		
		matchingAnswerCanvasElement.style.left = (parseFloat(matchingAnswerCanvasElement.style.left) + lLabelCanvasLeftPosition) + "px";
		answertextParentDiv.style.left = (parseFloat(answertextParentDiv.style.left) - lLabelCanvasLeftPosition) + "px";
		answertextParentDiv.style.width = (parseFloat(answertextParentDiv.style.width) + lLabelCanvasLeftPosition) + "px";
		answerLabelCanvasParentElement.style.left = "0px";
		answerLabelCanvasParentElement.style.top = (lLabelCanvasTopPosition) + "px";
		
		this.element.style.cursor = lCursorStyle;
		this.element.style.borderRadius = "5px"; 
		this.element.style.left = (-lLabelCanvasLeftPosition) + "px"; 
		this.element.style.top = "-2px";
		this.element.style.width = (this.answerTextCanvasImageBounds.maxX - this.answerTextCanvasImageBounds.minX + lLabelCanvasLeftPosition) + "px"; 
		this.element.style.height = (this.answerTextCanvasImageBounds.maxY - this.answerTextCanvasImageBounds.minY + 6) + "px";
		this.element.style.position = 'absolute';
		this.element.style.background = "#000000";
		this.element.style.opacity = 0;
		...
		lQuestionObj.registerColumn2Option(this.element);
		
		if(!this.visible)
			this.element.style.visibility = 'hidden';
	}
	
	cp.FIBAnswer = function(el, args)
	{
		cp.FIBAnswer.baseConstructor.call(this,el, args);
		...
		this.args = args;
		var lCaptionName = this.getAttribute("capN");
		var lCaptionData = cp.model.data[lCaptionName];
		var lCaptionCanvasData = cp.model.data[lCaptionData.mdi];
		this.sh = lCaptionCanvasData.sh;
	}
	
	cp.inherits(cp.FIBAnswer, cp.Answer);
	
	cp.FIBAnswer.prototype.addAndGetLabelCombo = function(lQuestionObj,iDivData)
	{		
		this.answerObjects = lQuestionObj.getAnswerObjects(this.element.id);		
		var lAnswerHolderElem = document.getElementById(iDivData.answerTextCanvasHolder);

		var lRelatedCaptionData = cp.model.data[iDivData.capN];
		this.relatedCaptionData = cp.model.data[lRelatedCaptionData.mdi];
		
		var lDropDownL = iDivData.b[0];
		var lDropDownT = iDivData.b[1];
		var lDropDownW = iDivData.b[2] - iDivData.b[0] + 1;
		var lDropDownH = iDivData.b[3] - iDivData.b[1];
		
		var labelDropDownElement = document.createElement("div");
		labelDropDownElement.id = this.element.id + "_labelDD";	
		labelDropDownElement.style.position = 'absolute';		
		labelDropDownElement.style.left = lDropDownL + "px";
		labelDropDownElement.style.top = lDropDownT + "px";
		labelDropDownElement.style.width = lDropDownW + "px";
		labelDropDownElement.style.height = lDropDownH + "px";
		...
		{						
			var innerHTMLStr = "";
			innerHTMLStr += "<select class='mydds' id='" + this.element.id + "_fibLabel' name='" + this.group + "'>";
			for(var z = 0; z < this.answerObjects.length; ++z)
			{
				var lAnswerObj = this.answerObjects[z];
				var lAnswerText = lAnswerObj.text;
				var lAnswerImageSrc = lAnswerObj.ip;
				
				innerHTMLStr += "<option value='" + lAnswerText + "' style='position:absolute;width:" + (lDropDownW) + "px' title='" + lAnswerImageSrc + "'>";
				
				if(cp.DESKTOP != cp.device)
				{					
					innerHTMLStr += lAnswerText;
				}
			...
			document.getElementById(labelDropDownElement.id).innerHTML = innerHTMLStr;
			var selectElement = document.getElementById(this.element.id + "_fibLabel");
			
			selectElement.style.width = (lDropDownW) + "px";
			selectElement.style.height = (lDropDownH) + "px";
			selectElement.style.position = 'absolute';
			selectElement.style.zIndex = 1000;
			var oHandler;
			if(cp.DESKTOP != cp.device)
			{					
				//console.log(lDropDownW);				
				var lTitleImageSrc;
				selectElement.style.opacity = 0;
				var self = this;
				selectElement.onchange = function(e)
				...
				var spaninnerStr = "<img id='" + this.element.id + "_spanImage' src='" + lDefaultTitleImagePath + "' title='" + lDefaultTitleImagePath + "' style='position:absolute;top:2px; left:5px; width:" + (lDropDownW - 10) + "px; height:" + (lDropDownH) + "px;'/>";
				spaninnerStr += "<img id='" + this.element.id + "_spanImageArrow' class='spanArrow' style='position:absolute;left:" + (lDropDownW - 20) + "px;top:3px'/>";
				
				var spanChild = document.createElement('span');
				spanChild.id='spanComboBoxTitleImage';
				spanChild.className = 'spanComboBox';
				spanChild.innerHTML = spaninnerStr;
				spanChild.style.width = (lDropDownW) + "px";
				spanChild.style.height = (lDropDownH + 3) + "px";
				spanChild.style.overflow = "none";
				this.answerLabelDivElem.appendChild(spanChild);	
				...
				{										
					if(iIndex != -1)
					{							
						self.answerObj = self.answerObjects[iIndex];
						lTitleImageSrc = self.answerObj.ip;						
						selectElement.selectedIndex = iIndex;
					}
					else
					{
				...
				this.answerLabelDivElem.style.position = 'absolute';
				this.answerLabelDivElem.style.left = lDropDownL + "px";
				this.answerLabelDivElem.style.top = lDropDownT + "px";
				this.answerLabelDivElem.style.width = lDropDownW + "px";
				this.answerLabelDivElem.style.height = lDropDownH + "px";
				
				if(this.sh)
				{
					var mAngle = this.sh.a;
					applyShadow(this.element.parentElement,this.sh.d*Math.cos((Math.PI*mAngle)/180) + 'px ' + this.sh.d*Math.sin((Math.PI*mAngle)/180) + 'px ' + this.sh.b + 'px '+ ConvertRGBToRGBA(this.sh.c,1 ) + (this.sh.i ? ' inset' : ''));				
				}
			...
			{
				var lLeftPosition = parseFloat(selectElement.style.left) + lDropDownL;
				var lTopPosition = parseFloat(selectElement.style.top) + lDropDownT;
				
				selectElement.style.position = 'absolute';
				selectElement.style.left = lDropDownL + "px";
				selectElement.style.top = lDropDownT + "px";
				selectElement.style.width = (lDropDownW) + "px";
				selectElement.style.height = lDropDownH + "px";
				
				var lSelectElement = jQuery("#" + this.element.id + "_fibLabel");
				lSelectElement.css("width",(lDropDownW));
				lSelectElement.css("height",(lDropDownH));
				oHandler = jQuery("#" + this.element.id + "_fibLabel").msDropDown({style:"position:fixed, z-index:1000;left:" + (lDropDownL + 20) + "px, top:" + (lDropDownT) + "px, height:" + (lDropDownH) + "px"},lDefaultTitleImagePath).data("dd");	
				oHandler.set("width",(lDropDownW));	
				oHandler.set("height",(lDropDownH));					
				oHandler.set("overflow",true);
				var self = this;
				var onClose = function()
										{
											var lIndex = self.element.oHandler.get("selectedIndex");
											if(lIndex != -1)
											{
												self.answerObj = self.answerObjects[lIndex];
											}
										};
				
				oHandler.addMyEvent("onClose",onClose);
				var jQueryDropDownElem = jQuery("#" + this.element.id + "_labelDD");
				if(this.sh)
				{	
					var mAngle = this.sh.a;
					applyShadow(jQueryDropDownElem[0],this.sh.d*Math.cos((Math.PI*mAngle)/180) + 'px ' + this.sh.d*Math.sin((Math.PI*mAngle)/180) + 'px ' + this.sh.b + 'px '+ ConvertRGBToRGBA(this.sh.c,1 ) + (this.sh.i ? ' inset' : ''));
				}
		...
		oHandler.getText = function()
		{			
			var lSelectedIndex = self.element.oHandler.getIndex();
			if(lSelectedIndex == -1)
				return "";
			return self.answerObjects[lSelectedIndex].text;
		};
		
		oHandler.setText = function(iText)
		{
			if(iText == "")
			{
				self.element.oHandler.setIndex(-1);
			...
			var selIndex = -1;
			for(var i = 0; i < self.answerObjects.length; ++i)
			{
				var lCurrAnsObj = self.answerObjects[i];
				if(lCurrAnsObj.text == iText)
				{
					selIndex = i;
					break;
				}
			}
			self.element.oHandler.setIndex(selIndex);
		};
		
		return oHandler;
	}
	
	cp.FIBAnswer.prototype.addAndGetTextInputField = function(lQuestionObj,iDivData)
	{
		var oHandler;
		var tag = 'input';
		var otherStyleStr = "";
		...
		if(this.element.innerHTML == "")
		{
			...
			var topPos = this.bounds.minY;
			var inputHeight = (this.bounds.maxY - this.bounds.minY);
			if ( cp.MSIE == cp.browser ) {
				topPos -= 1;
				inputHeight += 1;
			}
			this.element.innerHTML += "<" + tag + " type='text'" + (this.enabled ? "" : " readonly='readonly'") + " aria-label='" + this.accstring + "' id='" + this.element.id + "_inputField' style='left:0px; top:0px; width:" + (this.bounds.maxX - this.bounds.minX - 2) + "px; height:" + inputHeight + "px;position:absolute;" 
				+ otherStyleStr + "'></" + tag + ">";				
		}
		...
		{
			var mAngle = this.sh.a;
			applyShadow(this.inputField,this.sh.d*Math.cos((Math.PI*mAngle)/180) + 'px ' + this.sh.d*Math.sin((Math.PI*mAngle)/180) + 'px ' + this.sh.b + 'px '+ ConvertRGBToRGBA(this.sh.c,1 ) + (this.sh.i ? ' inset' : ''));				
		}
		
		var self = this;
		this.inputField.resetText = function()
		{
			self.element.oHandler.value = "";
		};
		
		this.inputField.setText = function(iText)
		{
			self.element.oHandler.value = iText;
		};

		this.inputField.getText = function()
		{					
			return self.element.oHandler.value;
		};

		this.inputField.disable = function()
		{
			self.enabled = false;
			self.element.oHandler.setAttribute("readonly",'readonly');
		};
		...
		oHandler = this.inputField;
		
		var lDropDownL = iDivData.b[0];
		var lDropDownT = iDivData.b[1];
		var lDropDownW = iDivData.b[2] - iDivData.b[0];
		var lDropDownH = iDivData.b[3] - iDivData.b[1];
	...
	cp.FIBAnswer.prototype.addIfNeeded = function()
	{
		var lQuestionObj = getQuestionObject(this.relatedQuestionSlide);
		if(!lQuestionObj)
			return;		
		...
		this.isComboBox = divData.sac;
		this.isCaseSensitive = divData.cs;
		this.correctAnswersList = divData.correctAnswers;
		var lReviewIconPositionLeft;
		var lReviewIconPositionTop;
		...
		this.answerLabelDivElem = document.getElementById(divData.dn);
		
		this.selectedText = lQuestionObj.getSelectedText(this.element.id);
		if(this.isComboBox)
		{
			//Show combo box here
			if(cp.verbose)
				cp.log("Draw Combo Box");		
			
			this.element.oHandler = this.addAndGetLabelCombo(lQuestionObj,divData);
		}
		else
		{
			//Show text field here
			if(cp.verbose)
				cp.log("Draw Text Field");
			
			this.element.oHandler = this.addAndGetTextInputField(lQuestionObj,divData);		
		}
		...
		this.element.clearAnswer = function()
		{
			self.selectedText = "";
			if(self.isDrawn)
				self.element.oHandler.setText("");
		}
		
		this.element.removeOpenReviewAreas = function()
		{
			var currSlide = document.getElementById(self.questionObj.getAssociatedObjName());
			var reviewAreaDiv = document.getElementById(self.element.id + "_correctfibReview");
			if(reviewAreaDiv != undefined)
				currSlide.parentNode.removeChild(reviewAreaDiv);
		};
		...
		this.element.addReviewButton = function()
		{
			...
			lReviewButton.style.position = 'absolute';
			var lReviewIcon;
			lReviewIcon = document.getElementById(self.element.id + "_reviewIncorrect");
			lReviewButton.style.left = (parseFloat(lReviewIcon.style.left) + lReviewIcon.clientWidth + 2) + "px";
			lReviewButton.style.top = lReviewIcon.style.top;
			lReviewButton.onclick = function(e)
							{	
								var lCorrectFIBDivId = lOptionElem.id + "_correctfibReview";
								var correctFIBDiv = document.getElementById(lCorrectFIBDivId);
								if(correctFIBDiv == undefined)
									correctFIBDiv = document.createElement('div');
									
								var innerHTMLStr = "<ul style='padding-left:20px;padding-right:20px'>";
																
								for(var i = 0; i < self.correctAnswersList.length; ++i)
								{
									innerHTMLStr += "<li>" + self.correctAnswersList[i] + "</li>";
								}
								
								innerHTMLStr += "</ul>"
								innerHTMLStr += "<img id='fibReviewCloseButton' src='./assets/htmlimages/closeReviewButton.png' style='right:6px;top:6px;position:absolute'>";
								correctFIBDiv.id = lCorrectFIBDivId;
								correctFIBDiv.className = 'sequenceReviewArea';
								correctFIBDiv.style.position = 'absolute';
								
								var lSelfReviewButton = document.getElementById(lOptionElem.id + "_reviewButton");
								correctFIBDiv.style.left = (parseFloat(lSelfReviewButton.style.left) + parseFloat(lSelfReviewButton.width) + 10) + "px";
								correctFIBDiv.style.top = 	parseFloat(lSelfReviewButton.style.top) + "px";
								correctFIBDiv.innerHTML = innerHTMLStr;
								
								if (e.stopPropagation) 
								{
									e.stopPropagation(); // stops the browser from redirecting.
								}
								self.questionObj.showReviewArea(correctFIBDiv);								
							};
		...
		this.lCorrectAnswersAreAlsoEmpty = false;
		for(var i = 0; i < this.correctAnswersList.length; ++i)
		{
			var lCorrAnsText = this.correctAnswersList[i];
			lCorrAnsText = lCorrAnsText.replace(/^\s*|\s*$/g,'');
			this.lCorrectAnswersAreAlsoEmpty = this.lCorrectAnswersAreAlsoEmpty || (lCorrAnsText == "");
		}
		...
		this.element.isAttempted = function()
		{
			if(self.isComboBox)
			{
				return (self.element.oHandler.getIndex() != -1);
			}
			else
			{			
				var lFinalText = self.element.oHandler.getText();
				lFinalText = lFinalText.replace(/^\s*|\s*$/g,'');
				return (lFinalText != "") || self.lCorrectAnswersAreAlsoEmpty;
			}
		...
		{
			var lText = "";
			var lText = self.element.oHandler.getText();
			lText = lText.replace(/^\s*|\s*$/g,'');
			if((lText == "") && !self.lCorrectAnswersAreAlsoEmpty)
				return false;					
						
			for(var i = 0; i < self.correctAnswersList.length; ++i)
			{
				var lCorrAnsText = self.correctAnswersList[i];
				lCorrAnsText = lCorrAnsText.replace(/^\s*|\s*$/g,'');
				if(!self.isCaseSensitive)
				{
					if( lCorrAnsText.toLowerCase() == lText.toLowerCase() )
						return true;
				}				
				else if(lCorrAnsText == lText)
				{
					return true;
				}
			}
		...
		lReviewIconPositionLeft = this.bounds.minX + "px";
		lReviewIconPositionTop = (this.bounds.maxY + 5) + "px";
		...
		if(lShouldDisable)
		{
			...
			if(lQuestionObj.getWasJudged() &&  lQuestionObj.canShowReviewIcons())
			{
				if(lQuestionObj.getAnsweredCorrectly())
				{
					...
					newImg.style.left = lReviewIconPositionLeft;
					newImg.style.top = lReviewIconPositionTop;					
					this.element.parentNode.parentNode.appendChild(newImg);
					if(self.sh)
					{
						var mAngle = self.sh.a;
						applyShadow(newImg,self.sh.d*Math.cos((Math.PI*mAngle)/180) + 'px ' + self.sh.d*Math.sin((Math.PI*mAngle)/180) + 'px ' + self.sh.b + 'px '+ ConvertRGBToRGBA(self.sh.c,self.sh.o ) + (self.sh.i ? ' inset' : '') + (self.sh.i ? ' inset' : ''));
				...
				{
					var imgSrc;
					if(!this.element.isAttempted())
						imgSrc = './assets/htmlimages/skip_answer_normal.png';
					else if(this.element.isCorrectlyAnswered())
						imgSrc = './assets/htmlimages/correct_answer_normal.png';					
					else
						imgSrc = './assets/htmlimages/incorrect_answer_normal.png';
					var newImg = document.createElement('img');
		...
		lQuestionObj.registerFIBAnswer(this.element);
		
		if(!this.visible)
			this.element.style.visibility = 'hidden';
	}
	
	cp.ShortAnswer = function(el, args)
	{
		cp.ShortAnswer.baseConstructor.call(this,el, args);
	...
	cp.ShortAnswer.prototype.addAndGetTextAreaField = function(lQuestionObj,iDivData)
	{
		var oHandler;
		var tag = 'input';
		var otherStyleStr = "";
		...
		if(this.element.innerHTML == "")
		{
			if ( this.font ) {
				...
				otherStyleStr += 'resize: none;overflow:auto;';
				otherStyleStr += 'border:1px solid;border-top-color:#696969;border-left-color:#696969;border-right-color:#E6E6E6;border-bottom-color:#E6E6E6;';
				otherStyleStr += 'border-radius:0px;padding:2px;box-sizing: border-box;';				
			}
			var topPos = this.bounds.minY;
			var inputHeight = (this.bounds.maxY - this.bounds.minY);
			if ( cp.MSIE == cp.browser ) {
			...
			this.element.innerHTML += "<textarea " + (this.enabled ? "" : " readonly='readonly'") + " aria-label='" + this.accstring + "' id='" + this.element.id + "_inputField' style='left:0px; top:0px; width:" + (this.bounds.maxX - this.bounds.minX) + "px; height:" + inputHeight + "px;position:absolute;" 
				+ otherStyleStr + "'></textarea>";				
		}
		this.inputField = document.getElementById(this.element.id + "_inputField");
	...
	cp.ShortAnswer.prototype.addIfNeeded = function()
	{
		var lQuestionObj = getQuestionObject(this.relatedQuestionSlide);
		if(!lQuestionObj)
			return;		
		...
		this.answerLabelDivElem = document.getElementById(divData.actid);
		...
		this.element.oHandler = this.addAndGetTextAreaField(lQuestionObj,divData);		
		...
		var self = this;
		this.questionObj = lQuestionObj;
		this.correctAnswersList = this.questionObj.getExpectedCorrectAnswerIDList();
		this.element.clearAnswer = function()
		{
			self.selectedText = "";
			if(self.isDrawn)
		...
		lQuestionObj.registerShortAnswer(this.element);
	...
	cp.TextInput = function(el, args)
	{
		...
		function submitTEBValue(event)
		{
			...
			if ( self.canBeAutoSubmitted || sc.isSame( eventSc ) ) {
				self.canBeAutoSubmitted = 0;
		...
		function autoSubmitTEBValue(event)
		{
			if(self.inputField.value.length >= self.m_charLimit && self.m_charLimit != -1)
			{
				self.canBeAutoSubmitted = 1;
				submitTEBValue(event);
			}
		...
		var lParentDivData = cp.model.data[this.parentDivName];
		this.transIn = 	lParentDivData['trin'];
		
		//new properties
		this.m_isAutoSubmit = lParentDivData.as;
		this.m_isLowercase = lParentDivData.lcase;
		this.m_isUppercase = lParentDivData.ucase;
		this.m_isNumeric = lParentDivData.num;
		this.m_hasNoCondition = lParentDivData.nc;
		this.m_charLimit = lParentDivData.cl;		
		...
		this.element.onkeypress = checkCharType;
		if(this.m_isAutoSubmit)
			this.element.onkeyup = autoSubmitTEBValue;
	...
	cp.TextInput.prototype.addIfNeeded = function()
	{
		...
		if(this.element.innerHTML == "")
		{
			if ( this.font ) {
				...
				otherStyleStr += 'font-size:' + this.font.s + 'px;';
				otherStyleStr += 'color: ' + this.font.c + ';';
				if (this.m_isUppercase)
					otherStyleStr += 'text-transform: uppercase;'; 
				else if (this.m_isLowercase)
					otherStyleStr += 'text-transform: lowercase;'; 
				if ( this.font.B )
					otherStyleStr += 'font-weight:bold;';
				if ( this.font.u )
					otherStyleStr += 'text-decoration: underline;';
		...
		if ( this.inputField ) {
			...
			if ( this.inputField.setSelectionRange ) 
				...
				this.inputField.setAttribute("maxlength",this.m_charLimit);
	...
	{
		var that = this;
		this.is_inside_canvas = function(e) 
		{     
			var lBool;
			if(!that.element)
				return;
			...
			if(lGc){
				...
				var lParentOffsetL = that.element.parentElement.offsetLeft;
				var lParentOffsetT = that.element.parentElement.offsetTop;
				var lElemL = parseFloat(that.element.style.left);
				var lElemT = parseFloat(that.element.style.top);
				var lElemMarginL = parseFloat(that.element.style.marginLeft);
				var lElemMarginT = parseFloat(that.element.style.marginTop);
				var X = lScaledPosition.X - (lElemMarginL < 0 ? lElemL : lParentOffsetL);
				var Y = lScaledPosition.Y - (lElemMarginT < 0 ? lElemT : lParentOffsetT);
				lBool = lGc.isPointInPath(X,Y);	
				return lBool;
			}
		...
		function doOnMouseMove( elem, data, iInsideCanvas )
		{
			if(iInsideCanvas)
			{
		...
		function getMouseHandler( event,elem, dataObj, handler, old_handler )
		{
			var old = old_handler;
			var e = elem;
			var data = {sx: dataObj.sx, sy: dataObj.sy, tx: dataObj.tx, ty: dataObj.ty, p: dataObj.p, old_tr: dataObj.old_tr};			
			return function(event) {
				var lInsideCanvas = that.is_inside_canvas(event);
		...
		if ( actualParent && undefined != this.parentData.pa && this.bounds ) {
			...
			if ( scaleX < 1.0 && scaleY < 1.0 ) {
				...
				if ( cp.device == cp.IDEVICE ) {
					actualParent.ontouchstart = getMouseHandler( event,this.element, dataObj, doOnMouseDown );
					actualParent.ontouchend = getMouseHandler( event,this.element, dataObj, doOnMouseUp );
					this.ontouchstartHandler = actualParent.ontouchstart;
					this.ontouchendHandler = actualParent.ontouchend;
				}
				else {
					actualParent.onmouseover = getMouseHandler( event,this.element, dataObj, doOnMouseUp, actualParent.onmouseover );
					actualParent.onmousemove = getMouseHandler( event,this.element, dataObj, doOnMouseMove, actualParent.onmousemove );
					actualParent.onmouseout = getMouseHandler( event,this.element, dataObj, doOnMouseUp, actualParent.onmouseout );
					actualParent.onmousedown = getMouseHandler( event,this.element, dataObj, doOnMouseDown );
					actualParent.onmouseup = getMouseHandler( event,this.element, dataObj, doOnMouseUp );
    ...
    cp.TypingText.prototype.addIfNeeded = function() {
        ...
        this.isDrawn = true;
		if(this.playKeyTap && cp.movie.playKeyTap)
		{
			cp.movie.am.playKeyTap();
		}
    }
	...
	cp.FrameSet.prototype.applyEffects = function()
	{
		...
		for(var index=0;index < this.itemData.runningEffects.length; ++index)
		{
			...
			if(!effectData.length)
				continue;
			var lAllEffectsFinished = true;
			var lShouldIncrementStartFrame = true;
			for(var i=0;i < effectData.length; ++i)
			{ 
				var effect = effectData[i];
				if(!effect)
				...
				if(this.itemData.rp)
					...
					from = this.from;
				var relFrame = cpInfoCurrentFrame;
				...
				if(effectData.startFrame!=undefined && lShouldIncrementStartFrame)
				{
					lShouldIncrementStartFrame = false;
				...
				if(effect.sf-1 > relFrame || effect.sf - 1 + effect.du <= relFrame)
				{
					...
					{
						lAllEffectsFinished = lAllEffectsFinished && true;
						if(lAllEffectsFinished)
						{
					...
					if(effect.sf-1 >= relFrame)
					{	
						lAllEffectsFinished = false;												
						continue;
					}
	...
	cp.FrameSet.prototype.reset = function(endOfSlide)
	{
		...
		if(endOfSlide)
		{
			...
			else if(this.children && this.children[0] && this.children[0].continueToNextSlide)
				setDisplayNone = false;
		}
	...
	cp.Feedback.prototype = 
	{
		...
		{			
			...
			var lRuleName = this.itemName + "_rule";;
			function ruleExists()
			{
				var ss = document.styleSheets[0];
				if(ss && !ss.cssRules)
					return false;
				//Assumption CPLibrary.css should be the first CSS
				// loop through all the rules of CPLibrary.css
				for (var j = 0; j < ss.cssRules.length; ++j) 
				{						
					// find the -webkit-keyframe rule whose name matches our passed over parameter and return that rule
					if (ss.cssRules[j].type == window.CSSRule.WEBKIT_KEYFRAMES_RULE && ss.cssRules[j].name == lRuleName)
						return true;
				}				
			...
			if(ruleExists())
			{
				if(cp.verbose)
					cp.log("rule exists");
				this.webkitAnimationRule = lRuleName;
				return;
			}
			
			var lRuleStr = "";
			if(lItemData.trin != 0 && lItemData.trin != undefined)
			{				
				var lTrinPercent = Math.round(lItemData.trin*10000/lItemData.du)/100;
				lRuleStr += " { 0% { opacity:0; } " + lTrinPercent + "% { opacity:1; } ";
			}
			else
			{
				lRuleStr += " { 0% { opacity:1; } ";
			}			
			if(lItemData.trout != 0 && lItemData.trout != undefined)
			{
				var lTroutPercent = Math.round((lItemData.du - lItemData.trout)*10000/lItemData.du)/100;
				lRuleStr += " " + lTroutPercent + "% { opacity:1; } " + " 100% { opacity:0; } }";
			}
			else
			{
				lRuleStr += " 100% { opacity:1; } }";
			}
			//CPLibrary.css is the first CSS.
			var firstSheet = document.styleSheets[0];
			if(cp.verbose)
				cp.log(lRuleName + ", " + lRuleStr);
			firstSheet.insertRule("@-webkit-keyframes " + lRuleName + lRuleStr);
		...
		show: function()
		{
			...
			if(this.drawingBoard)
			{
				if(cp.canUseWebkitAnimations() && !(cp.FeedbackType.HINT == this.type))
				{
					...
					this.drawingBoard.style.webkitAnimationDuration = lItemData.du/cp.movie.fps + 's';	
				}
				else
				{
					if(cp.device != cp.DESKTOP) //for the devices with webkit version lower than expected, no animation will be shown.
	...
	cp.Timeline.prototype.setupSlideItemDiv = function(item, itemData, slide, slideDiv, itemDiv, slideCanvas)
	{
		...
		else if ( cp.kCPOTOvalItem == item.t || cp.kCPOTRectangleItem == item.t || cp.kCPOTPolygon == item.t || cp.kCPOTAnswerArea == item.t || cp.kCPOTMatchingQuestionArea == item.t || cp.kCPOTMatchingAnswerArea == item.t) {
    ...
    cp.Timeline.prototype.setupAccessibility = function(inItem, inDiv)
	{
	    if(inItem)
		{
			...
			if ((inItem.t == cp.kCPOTCaptionItem) || (inItem.t == cp.kCPOTScoringResultItem) || ( cp.kCPOTSuccessCaptionItem == inItem.t ) 
				...
				|| ( cp.kCPOTStageMatchingQuestion == inItem.t ) || ( cp.kCPOTStageMatchingAnswer == inItem.t ) || ( cp.kCPOTStageMatchingAnswerEntry == inItem.t ) || ( cp.kCPOTStageAnswerItem == inItem.t ) || ( cp.kCPOTQuestionColumn == inItem.t ) )
			{
				inDiv.setAttribute('tabindex', '0');
				if((cp.kCPOTFillBlankCaption == inItem.t))
					inDiv.setAttribute('aria-label',lItemData['fibText']);
				else
	...
	cp.Timeline.prototype.addFramesetsAndLoadAssetsForSlideAtIndex = function(slideIndex)
	{
		...
		var interactions = slideData.int;
		...
		{					
			...
			if( (newFrame >= from) && (newFrame <= to) )
			{
				...
				try
				{
					...
					var ddim = this.m_interactionManagers[tempSlideName];
					if( ddim )
					{
	...
	cp.Timeline.prototype.updateFrame = function(reasonForUpdate)
	{
		...
		if(this.cEnd == cpInfoCurrentFrame)
		{
			...
			if(typeof(cptb) != 'undefined')
			{
				if(cptb.onSlideExit)
				{
					cptb.onSlideExit(cp.model.data[this.slides[cpInfoCurrentSlideIndex]]);
				}
			}
		...
		if( (this.cEnd < cpInfoCurrentFrame) || (this.cStart > cpInfoCurrentFrame))
		{
			...
			if(typeof(cptb) != 'undefined'){
				...
				{
					cptb.onSlideEnter(cp.model.data[this.slides[cpInfoCurrentSlideIndex]]);
				}
			}
	...
	cp.Timeline.prototype.attemptToAdvanceCurrentFrameByOffset = function(elapsedFrames)
	{
		...
		setCpInfoCurrentFrame(newFrame, smoothAdvance);
	}
	...
	cp.AudioObject.prototype = 
	{
		...
		play: function()
		{
			...
			if(!this.paused)
			{
				if(!cp.multiAudioTrack && this.nativeAudio)
				{
		...
		pause: function()
		{
			...
			if(this.duration != undefined && this.duration < 0.5)
				return;
	...
	cp.AudioManager.prototype = 
	{
		pendingAudios: function()
		{
			...
			for(var i = 0; i < this.MAX_AUDIO_CHANNELS; ++i)
			{
				...
				if(a.waitCount > 0 && !a.endedAt && !a.paused)
					++numPending;
			}
		...
		{
			if(cp.IDEVICE == cp.device && cp.multiAudioTrack)
			{
				var freeSlots = 0;
				for(var i = 0; i < this.MAX_AUDIO_CHANNELS; ++i)
				{
		...
		play:function(frame)
		{
			...
			if(cp.device == cp.IDEVICE && audioObj && !cp.multiAudioTrack)
			{
		...
		{
			if(play && cp.device == cp.IDEVICE && !cp.multiAudioTrack)
			{
...
cp.MOVIEFOCUSINEVENT = 14;
	...
	function updateNoSkipFramesAndUpdateVarText(cpElement)
	{
		...
		if(cpElement.effectObj && cpElement.itemData && cpElement.itemData.runningEffects && cpElement.itemData.runningEffects.length)
		{
			for(var index=0;index < cpElement.itemData.runningEffects.length; ++index)
			{
				var efId = 'ef'+cpElement.itemData.runningEffects[index];
				var effectData = cpElement.effectObj[efId];
				if(!effectData)
					continue;
				for(var i=0;i < effectData.length; ++i)
				{ 
					...
					var from;
					if(cpElement.itemData.rp)
						from = cp.movie.stage.cStart;
					else
						from = cpElement.from;
					var effectEnd = (from - 1) + (effect.sf - 1) + effect.du;
					cp.movie.stage.noSkipFrames[effectEnd] = effectEnd;
				}
			}
		}
	}
	...
	function setCpCmndShowPlaybar(inShow)
	{
		...
		{
			playbar.style.backgroundColor = cp.model.data.project.htmlBgColor;
	...
	function setCpInfoCurrentFrame(aCurrFrame, dontForce)
	{
	...
	cp.EventVideo.prototype.start = function()
	{
		function _start(video)
		{
			if(cp.device == cp.IDEVICE && cp.multiAudioTrack)
			{
				var delay = 500;//mS
				var from = video.from;
				if(from < cpInfoCurrentFrame)
					from = cpInfoCurrentFrame;
				var to = from + (delay/1000)*cpInfoFPS;
				if(to > video.to)
				{
					to = video.to;
					//delay = (video.to - from)*1000/cpInfoFPS;
				}
				if(cp.movie.am.doesAudioStartBetween(from, to))
				{
					if(video.vdm.verbose)
						cp.log(delay +'mS delay in start of ' + video.id);
					video.delayStartTimer = setTimeout(function(){_start(video);}, delay);
					return;
				}
			}
			video.drawIfNeeded();
			video.addNativeVideoIfNeeded();
		}
	...
	cp.EventVideo.prototype.addNativeVideoIfNeeded = function()
	{
		...
		if(this.nativeVideo)
		{
			...
			if(cp.device == cp.IDEVICE && cp.IOSMajor >= cp.IOS5)
			{
	...
	cp.SlideVideo.prototype.addNativeVideoIfNeeded = function()
	{
		...
		if(this.showInTOC)
		{
			...
			if(sourceChanged && cp.device == cp.IDEVICE && cp.IOSMajor >= cp.IOS5)
			{
				this.nativeVideo.CPcanPlay = false;
	...
	cp.VideoManager.prototype = 
	{
		pendingVideos: function()
		{
			...
			for(var i = 0; i < this.MAX_VIDEO_CHANNELS; ++i)
			{
				...
				if(v.waitCount > 0 && !v.endedAt && !v.paused)
					++numPending;
			}
				...
				for(var i = 0; i < slide.videos.length; ++i)
				{
					...
					else if(v.type == cp.kCPOTVideo)
					{
						...
						if(vc.sit)//toc
						{
							...
							if(this.tocVideoChannel.cpSrc == vc.mp4)
							{
								if(this.verbose)
									cp.log('vdm preload found existing ' + vc.mp4);
								continue;
							}
							if(!this.tocVideoChannel.cpVideo)
							{
								this.tocVideoChannel.cpSrc = vc.mp4;
								this.tocVideoChannel.src = vc.mp4;
								this.tocVideoChannel.load();
								if(this.verbose)
									cp.log('vdm preloaded tocV with ' + vc.mp4);
							}
						...
						else
						{
							...
							if(this.slideVideoChannel.cpSrc == vc.mp4)
							{
								if(this.verbose)
									cp.log('vdm preload found existing ' + vc.mp4);
								continue;
							}
							if(!this.slideVideoChannel.cpVideo)
							{
								this.slideVideoChannel.cpSrc = vc.mp4;
								this.slideVideoChannel.src = vc.mp4;
								this.slideVideoChannel.load();
								if(this.verbose)
									cp.log('vdm preloaded SV with ' + vc.mp4);
							}
		...
		{
			if(cp.device == cp.IDEVICE)
				return this.preload1(slideName)
			else
...
function getCpInfoOriginalFPS()
{
	return cp.movie.fps;
}
function getCpInfoSpeed()
{
	return cp.movie.speed;
}
function getCpElapsedMovieTime()
{
	return cp.movie.elapsedMovieTime;
}
function getCpIsPlaying()
{
	return !cp.movie.paused;
}
...
function vTEB( tebDivName )
{
	...
	if ( isCorrect ) { 
		...
		SubmitInteractions(tebDivName, cp.QuestionStatusEnum.CORRECT, currentAttempt);
	} 
	else { 
		feedbackType = cp.FeedbackType.FAILURE;
		caption = failureCaption; 
		action = failureAction; 
		SubmitInteractions(tebDivName, cp.QuestionStatusEnum.INCORRECT, currentAttempt);
	}
	...
	if ( captionToBeShown ) {
		...
		if(!pauseMovie && shouldExecuteAction)
			cp.movie.executeAction( action ); 
		feedback.show();
	} 		 
...
function wch(e) {return cp.HandleInteractiveWidget(e);}

function SubmitInteractions(objName,isCorrectEnum,currentAttempt,iCorrectValues,iCurrentValue)
{
...
function handleClickExternal(e)
{
	...
	for(var i=children.length - 1; i >= 0; --i)
	{
		...
		if(IsPointWithin())
		{
			...
			if(cp.kCPOTAutoShape == clickItem.type) 
			{
				...
				{
					if(!currElementDivData.isCanvasClicked(e))
					{
						ShouldShowFailure();
						continue;
					}
			...
			if(currElementDivData['enabled'])
			{
				...
				SubmitInteractions(currElement.id, cp.QuestionStatusEnum.CORRECT, currentAttempt);
	...
	if ( shouldShowFailure ) {
		...
		if(clickItem)
		{
			...
			SubmitInteractions(lItemId, cp.QuestionStatusEnum.INCORRECT, currentAttempt - 1);
		}
...
function InitMediaAndBeginMovie()
{
...
function setFillStrokeStyle(fillObj, canvasElem, stroke, shouldNotSetAlpha)
{
...
function CPPreInit()
{
	function CPInitDebugControls()
	{
		if(cp.dynamicLogControl)
		{
			...
			var stopAudioWait = document.createElement('div');
			stopAudioWait.style.cssText = "position:absolute;left:120px; top:60px; width:20px; height:20px;display:block;background-image:url(assets/htmlimages/hotspot_correct_answer_normal.png);opacity:1";
			...
			var stopVideoWait = document.createElement('div');
			stopVideoWait.style.cssText = "position:absolute;left:120px; top:90px; width:20px; height:20px;display:block;background-image:url(assets/htmlimages/hotspot_correct_answer_normal.png);opacity:1";
...
function SetScaleAndPosition()
{
	...
	if(window.parent != undefined)
		lWindowToRescale = window.parent;
	...
	if(!(lPlaybar.style.display == "none" || lPlaybar.style.visibility == "hidden") && (cp.model.data.playBarProperties.hasPlayBar))
	{
		...
		{			
			...
			var lCurrPlaybarH = 0;
			var lCurrPlaybarW = 0;
			if(lPlaybarBackground)
			{
				lCurrPlaybarH = parseFloat(lPlaybarBackground.clientHeight);
				lCurrPlaybarW = parseFloat(lPlaybarBackground.clientWidth);	
			}
	...
	var lToc = document.getElementById('toc');
	var lTocWidth = 0;
	if(lToc && !(lToc.style.display == "none" || lToc.style.visibility == "hidden") && cp.model.data.project.hasTOC)
	{
		lTocWidth = !cp.model.data.tocProperties.overlay ? cp.model.data.tocProperties.width : 0;//(cp.model.data.tocProperties.position == 1 ? cp.model.data.tocProperties.width : 0) : 0;
	}
...
function getLeftTopAfterTranslate(iDivElem)
{

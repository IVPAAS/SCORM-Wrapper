...
function CheckForDebugCommandOnDevices(e)
{
	if(e.rotation > 30)
		LMSDriverHolder.ShowDebugWindow();
}
...
{	
	...
	try
	{
		...
		if(!LMSDriverHolder.blockedForLMS)
		{
			LMSDriverHolder.blockedForLMS = true;
		...
		{
			document.ongesturechange = CheckForDebugCommandOnDevices;
		}
	}
	catch(e)
	{
...
cp.PlaybackController.prototype = 
{
	...
	CreateTrackingAdapter: function()
	{
		...
		{
			this.m_trackingAdapter = new cp.Breeze();
			window.onbeforeunload = UnloadActivties;
		}
		else
		...
		switch(this.m_interaction_ary[ia_int].type_str)
		{
			...
			case "matching":
		        ...
		        for (resp_idx = 0; resp_idx < temp_responses_arr.length; ++resp_idx) {
		            ...
		            var resp_src = LMSDriverHolder.CreateResponseIdentifier(match_temp1, lLongLeftColText);
		            var resp_target = LMSDriverHolder.CreateResponseIdentifier(match_temp2, lLongRightColText);

		            
		            correctResponse_MatchingResponses_arr.push(new LMSDriverHolder.MatchingResponse(resp_src, resp_target));
		        }
		        ...
		        for (resp_idx = 0; resp_idx < temp_responses_arr.length; ++resp_idx) {
		            ...
		            studentResponse_MatchingResponses_arr.push(new LMSDriverHolder.MatchingResponse(resp_src, resp_target));
		        }

		        var lResultBln = ConvertToInteractionResultConstant(this.m_interaction_ary[ia_int]["result_str"]);
...
{	
	...
	switch(iFunction_str)
	{
		...
		{
			LMSDriverHolder.RecordMatchingInteraction(iArgsArr[0],iArgsArr[1],iArgsArr[2],iArgsArr[3],iArgsArr[4],iArgsArr[5],iArgsArr[6],iArgsArr[7]);
			break;
		}
		default :
		{
...
function isBlockedForLMS()
{
	return ((typeof(LMSDriverHolder) != 'undefined') && LMSDriverHolder && LMSDriverHolder.blockedForLMS);
}

function unblockFromLMS()
{
	if(LMSDriverHolder.blockedForLMS)
	{
		LMSDriverHolder.blockedForLMS = false;

...
cp.Widget = function (el, args) {
    ...
    var  lInteractiveWidget = this.getAttribute("iiw");	
    var  widDivName = this.getAttribute("dn");	
    var  widDivData = cp.model.data[widDivName];
    var  hintCaptionName =  widDivData.dep?widDivData.dep[0]:"";
    ...
    this.captivate = {
        CPMovieHandle : {
			...
			{
				if(!lInteractiveWidget)
					return 0;
			
				return{
					shouldDisable: function()
					{
						var lQuestionObj = getQuestionObject(widDivName);
						if(lQuestionObj)
						{
							return lQuestionObj.shouldDisableOptions();
						}
						
						return false;
					},
					setSuccess: function(bOK,iCorrectValues,iCurrentValue)
					{
						var obj = widDivData;
						if(!obj)
							return;								
						...
						{
							//Success		
							SubmitInteractions(lObjId, cp.QuestionStatusEnum.CORRECT, lCurrentAttempt, lCorrectValues, lCurrentValue);
							cp.clickSuccessHandler(widDivData);
							obj.handled = true;
							return;
						}
						else
						{
							...
							if(lObjId)
								SubmitInteractions(lObjId, cp.QuestionStatusEnum.INCORRECT, lCurrentAttempt-1, lCorrectValues, lCurrentValue);	
							cp.clickFailureHandler(obj, shouldExecuteAction);
							if ( ! obj.handled )
					...
					{
						//alert("Hint Caption to be Shown");
						cp.showHint(hintCaptionName,cp.movie.stage);
					},
					setHideHint: function()
					{
						//alert("Hint Caption to be hidden");
						cp.hideHint(hintCaptionName,cp.movie.stage);
					}
				}
			},
			...
			getCPQuestionData : function(){
				...
				var lCPQuestionDataObject = new Object();
				lCPQuestionDataObject.questionNumInQuiz	= lQuestionObj.getQuestionNumberInQuiz();
				lCPQuestionDataObject.weightage =  lQuestionObj.getWeighting();				
				return lCPQuestionDataObject;
			}
        }
    };
...
cp.Widget.prototype.start = function () {
	...
	if(!this.visible)
		...
		cp.redrawItem(this.parentDivName);
	cp.Widget.superClass.start.call(this);
};

cp.Widget.prototype.HandleClickEvent = function(e)
{
	if(this.widget && this.widget.onClickExternal)
		this.widget.onClickExternal(e);
};
	...
	if (this.iframe) {
		...
		if (type) {
			try {
				...
				{
					this.widget.onLoad();
					//Check if this is question widget
					var lIsTrackableWidget = this.getAttribute("iqw") || this.getAttribute("iiw");
					var  lWidgetDivName = this.getAttribute("dn");
					if(lIsTrackableWidget && lWidgetDivName)
					{	
						var lQuestionObj = getQuestionObject(lWidgetDivName);
						if(lQuestionObj && lQuestionObj.setWidget)
							lQuestionObj.setWidget(this.widget);
					}
				}
			} catch (e) {
...
cp.Widget.prototype.reset = function () {
    ...
    try {
		...
		{	
			var lQuestionObj = getQuestionObject(lWidgetDivName);
			if(lQuestionObj && lQuestionObj.resetWidget)
				lQuestionObj.resetWidget();
		}
